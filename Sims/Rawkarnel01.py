...
print("🌌 ALADIN Plasma Cosmology v39.0 — STABLE FUSED RAWKERNEL CT YEE (NaN Fixed)")
...
v_phi_factor = 0.12
c_h_factor = 6.0    # Stronger cleaning
kappa_factor = 5.0  # Much stronger damping
...
# ====================== FULL FUSED RAWKERNEL ======================
kernel_code = r'''
extern "C" __global__
void fused_ct_yee_stable(float* vx, float* vy, float* vz,
                         ...
                         float dt, float dx, int N, float c_h, float kappa)
{
    ...
    if (i < N-1 && j < N-1 && k < N-1) {
        ...
        float Ez_val = 0.05f * (-(vx_avg * By_avg - vy_avg * Bx_avg));  // Reduced strength
    ...
    if (i < N && j < N && k < N) {
        int g = (i*N + j)*N + k;
        psi[g] -= dt * kappa * psi[g];   // Strong damping
    }
}
'''

kernel = cp.RawKernel(kernel_code, 'fused_ct_yee_stable')
...
print("Starting v39.0 — Stable fused CT Yee...")
...
for step in range(steps):
    dt = CFL * dx / 250.0   # Safer timestep
    kernel(grid, block, (vx, vy, vz, Bx, By, Bz, psi, dt, dx, N, c_h, kappa), shared_mem=shared_bytes)

    # NaN protection
    Bx = cp.nan_to_num(Bx, nan=0.0)
    By = cp.nan_to_num(By, nan=0.0)
    Bz = cp.nan_to_num(Bz, nan=0.0)
    psi = cp.nan_to_num(psi, nan=0.0)
...
print("\n✅ v39.0 Finished! Fused RawKernel with stability fixes.")
