...
# Install (run first if needed)
# !pip install --upgrade "jax[cuda12]" matplotlib pyvista scipy tqdm -q
...
# Analytic m=3 kink growth rate (simplified)
k_z = CONFIG['k_pert'] / 0.1
v_A_approx = CONFIG['v_max'] * 0.5
gamma_analytic = k_z * v_A_approx * np.sqrt(3 * 2 / 4)
...
def hydro_hll_flux(UL, UR):
    ...
    SL = jnp.min(vL - csL, vR - csR)
    SR = jnp.max(vL + csL, vR + csR)
