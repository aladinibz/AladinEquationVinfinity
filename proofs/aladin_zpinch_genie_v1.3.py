...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.3 – 100,000 steps + STRONG 43 Hz PUMP           ║
# ║  (128³ grid, max dt ramp, Majorana domination, forced resonance)           ║
# ╚════════════════════════════════════════════════════════════════════════════╝

TEST_MODE = True

GRID_SIZE = 128
MAX_STEPS = 100000         # 100k steps – the big one
dt_max    = 5e-5           # 50 µs ceiling
CFL       = 0.55

print("🚀 ALADIN v1.3 – 100,000 steps + STRONG 43 Hz PUMP (0.05 amplitude)")
print("This should finally show a clear 43 Hz peak in the spectrum!")
print(f"dt_max = {dt_max:.1e}, CFL = {CFL}, expected runtime ~45–90 min on A100")
...
y_ferm = 1e-3
g_genie = 8e-4
kg_damping = 0.18          # lowered for sharper resonance
C_field_pump = 1e-3
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.3 – 100k steps + STRONG 43 Hz PUMP")
...
while t < t_max and step < MAX_STEPS:
    ...
    # Aggressive ramp-up for first 15k steps
    if step < 15000 and t < 0.1:
        dt_new = cp.maximum(dt_new, 3e-4)
        if t < 0.02:
            dt_new = cp.minimum(dt_new * 1.9, 6e-4)
    ...
    # Genie scalar + STRONG EXPLICIT 43 Hz PUMP
    source_genie = g_genie * J_z + majorana_backreaction
    source_genie += 0.05 * cp.sin(2 * cp.pi * 43 * t)   # ← STRONG 43 Hz pump (0.05 amplitude)
    source_genie = cp.clip(source_genie, -2000, 2000)
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.3 – 100,000 steps – FINAL ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    plt.title(f'43 Hz Resonance Detection – 100000 steps – {verdict}', color='white')
    plt.legend()
    plt.grid(alpha=0.25, which='both', color='gray')
    plt.savefig('plots/plot43hz_detection_v13_100000steps.png', dpi=220, facecolor='black')
    plt.close()
    print("Saved: plots/plot43hz_detection_v13_100000steps.png")
else:
    print("Not enough time points for 43 Hz FFT")
...
plt.ylabel('Power')
plt.title('Fermion Mass Spatial Power Spectrum – 100000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_100000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_100000.png")
...
plt.xlabel('Wavenumber k (1/a)')
plt.ylabel('Power')
plt.title('Fermion Density Spatial Power Spectrum – 100000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot44_fermion_density_spatial_spectrum_100000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot44_fermion_density_spatial_spectrum_100000.png")

print("\nALADIN v1.3 – 100,000 steps finished!")
print("The strong 43 Hz pump is now active – expect a clear peak!")
print("Drop the final time span, verdict, dominant k, Majorana %, genie amp, and any plot observations.")
