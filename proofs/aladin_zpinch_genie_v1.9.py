...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.9 – 700,000 steps – SYMMETRY BREAKER           ║
# ║  (128³ grid, Bessel J₀(kr) + multi-mode kink seed m=1–8, zero diffusion)   ║
# ╚════════════════════════════════════════════════════════════════════════════╝

GRID_SIZE = 128
MAX_STEPS = 700000         # 700k steps – balanced overnight push
dt_max    = 1.2e-4
CFL       = 0.75

print("🚀 ALADIN v1.9 – 700k steps – SYMMETRY BREAKER NIGHT RUN")
print("Bessel J₀(kr) + multi-mode kink seed (m=1–8) to break symmetry & trigger coupling!")
print(f"dt_max = {dt_max:.1e}, CFL = {CFL}, expected runtime ~4–6 hours on A100")
...
k_sausage = cp.float64(0.7)  # repo-tuned fastest sausage growth
...
# Initial conditions – BESSEL J₀(kr) + MULTI-MODE KINK SEED (symmetry breaker)
...
# Explicit multi-mode kink seed (m=1–8, mild amplitude) – symmetry breaker
m_kink = [1, 2, 3, 5, 8]
for m in m_kink:
...
# Histories – DOWNSAMPLE every 1000 steps to save RAM
...
try:
    while t < t_max and step < MAX_STEPS:
        ...
        # Fermion + Majorana – STRONGER BACKREACTION
        ...
        majorana_backreaction = 1e-3 * ferm_total_mass  # stronger push

        # Downsample history every 1000 steps
        if step % 1000 == 0:
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.9 – 900,000 steps – FINAL ANALYSIS")
print("="*80)
...
# 43 Hz detection with bandpass – safety check
if len(genie_center_history) > 33:
    ...
    plt.grid(alpha=0.3)
    plt.legend()
    plt.savefig('plots/plot35_filtered_43hz_signals_900k.png', dpi=200)
    plt.close()
    print("Saved: plots/plot35_filtered_43hz_signals_900k.png")
else:
...
plt.ylabel('Power')
plt.title('Fermion Mass z-Spatial Power Spectrum – 900k steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_900k.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_900k.png")
...
plt.ylabel('Power')
plt.title('Azimuthal Power Spectrum – Kink Detection (900k)')
plt.axvline(1, color='red', ls='--', label='m=1 kink')
plt.legend()
plt.grid(alpha=0.3)
plt.savefig('plots/plot45_azimuthal_power_kink_900k.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot45_azimuthal_power_kink_900k.png")

print("\nALADIN v1.9 – 900,000 steps finished!")
