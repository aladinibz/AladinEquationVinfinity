"""
ALADIN v1.1.7 — 128³ LOCKED - Ultra Safe Scalar Fixes
"""
...
print("🚀 ALADIN v1.1.7 — 128³ LOCKED - Ultra Safe\n")
...
for J0 in J0_list:
    for rho0 in rho0_list:
        ...
        for step in tqdm(range(n_steps), leave=False, desc=f"J0={J0:.1e}"):
            ...
            # ULTRA SAFE scalar conversion
            max_speed = max(float(vA.max().item()), float(vfluid.max().item()), 1e4)
            ...
            if step % save_every == 0:
                ...
                m0 = float(power_m[0].item()) if len(power_m) > 0 else 0.0
                m1 = float(power_m[1].item()) if len(power_m) > 1 else 0.0
