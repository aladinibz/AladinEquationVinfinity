"""
ALADIN v1.1.7 — 128³ LOCKED - Ultra Safe for Colab
"""
...
print("🚀 ALADIN v1.1.7 — 128³ LOCKED - Safe Version\n")
...
for J0 in J0_list:
    for rho0 in rho0_list:
        ...
        for step in tqdm(range(n_steps), leave=False, desc=f"J0={J0:.1e}"):
            ...
            max_vA = float(vA.max().item())
            max_vfluid = float(vfluid.max().item())
            max_speed = max(max_vA, max_vfluid, 1e4)
