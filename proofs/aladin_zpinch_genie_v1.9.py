...
from scipy.special import j0  # <--- added for Bessel J0

GRID_SIZE = 128
MAX_STEPS = 500000         # keep 500k – the length that worked
dt_max    = 1.2e-4
CFL       = 0.75

print("🚀 ALADIN v1.6 – 500,000 steps – FULL SPATIAL PUSH WITH FIXES")
print("Bessel J₀(kr) radial seed + multi-mode kink seed to break symmetry")
print(f"dt_max = {dt_max:.1e}, CFL = {CFL}, expected runtime ~4–8 hours on A100")
...
k_sausage = cp.float64(0.7)  # repo-tuned for fastest sausage growth
...
# FIX 1: Bessel J0(kr) radial profile for sausage eigenmode (from your repo proofs)
r_1d = r.get()  # numpy for j0
j0_kr = j0(k_sausage * r_1d)
j0_kr = cp.array(j0_kr)
j0_kr = cp.maximum(j0_kr, 0.0)  # clip negative lobes (optional, your proofs sometimes do)
j0_kr /= cp.max(cp.abs(j0_kr))  # normalize max = 1

j0_3d = j0_kr[:, cp.newaxis, cp.newaxis]
rho += 1.0 * rho0 * j0_3d * cp.cos(k_sausage * Z)  # <--- replaced simple cos with J0(kr) * cos(kz)

# FIX 2: Explicit multi-mode kink seed (m=1–8, mild amplitude) – symmetry breaker from your friend
m_kink = [1, 2, 3, 5, 8]
for m in m_kink:
    rho += 0.08 * rho0 * cp.cos(m * Phi) * cp.cos(k_sausage * Z)  # helical sausage beads
    v_phi += 0.03 * vA * cp.sin(m * Phi) * cp.sin(k_sausage * Z)  # azimuthal velocity twist
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.6 – 500k steps – SAUSAGE & KINK MAXIMUM")
...
while t < t_max and step < MAX_STEPS:
    ...
    genie_phi_new -= 0.0001 * cyl_laplacian(genie_phi_new) * dt  # ultra-weak diffusion
...
# =============================================================================
# POST-PROCESSING – with azimuthal kink analysis
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.6 – 500,000 steps – FINAL ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    plt.grid(alpha=0.3)
    plt.legend()
    plt.savefig('plots/plot35_filtered_43hz_signals_500k.png', dpi=200)
    plt.close()
    print("Saved: plots/plot35_filtered_43hz_signals_500k.png")
...
plt.ylabel('Power')
plt.title('Fermion Mass z-Spatial Power Spectrum – 500k steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_500k.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_500k.png")
...
plt.ylabel('Power')
plt.title('Azimuthal Power Spectrum – Kink Detection (500k)')
plt.axvline(1, color='red', ls='--', label='m=1 kink')
plt.legend()
plt.grid(alpha=0.3)
plt.savefig('plots/plot45_azimuthal_power_kink_500k.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot45_azimuthal_power_kink_500k.png")

print("\nALADIN v1.6 – 500,000 steps finished!")
print("Strong sausage + kink seeds active – check sausage band and m=1 power!")
