...
print("🌌 Plasma Cosmology v33.0 — FULL COMPLETE CODE (STAGGERED SEEDING FIXED + KINETIC KEPT)")
...
# ====================== FIXED STAGGERED B-FIELD SEEDING (PAD FOR ALL THREE) ======================
B0 = 5.0
Bphi = 2.0 * cp.exp(-r_cyl / 12.0)

# Expand cell-centered r_cyl to match each staggered face shape
r_cyl_Bx = cp.pad(r_cyl, ((0,1),(0,0),(0,0)), mode='edge')   # Bx (N+1, N, N)
r_cyl_By = cp.pad(r_cyl, ((0,0),(0,1),(0,0)), mode='edge')   # By (N, N+1, N)
r_cyl_Bz = cp.pad(r_cyl, ((0,0),(0,0),(0,1)), mode='edge')   # Bz (N, N, N+1)

Bz += B0 * cp.exp(-r_cyl_Bz**2 / 200.0)
Bx -= Bphi * (Y / (r_cyl_Bx + 1e-8))
By += Bphi * (X / (r_cyl_By + 1e-8))
...
# ====================== KINETIC ENERGY (EXPLICITLY KEPT FOR GALACTIC ROTATION) ======================
