...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.3 – Fast Iteration + Fermion Spectra            ║
# ║  (128³ default, visible Majorana, 43 Hz detection, spatial fermion spectra)║
# ╚════════════════════════════════════════════════════════════════════════════╝
...
if TEST_MODE:
    GRID_SIZE = 128
    MAX_STEPS = 1200
    print("TEST MODE ACTIVE: 128³ grid + 1200 steps (fast tuning ~1–2 min)")
else:
    GRID_SIZE = 256
    MAX_STEPS = 3000
    print("FULL MODE: 256³ grid + 3000 steps (high quality)")
...
# =============================================================================
# PARAMETERS – v1.3 tuned for visible dynamics
# =============================================================================
...
kappa_maj = 0.04              # visible quadratic Majorana
y_ferm = 1e-3
g_genie = 8e-4                # main growth driver
kg_damping = 0.25             # allow amplitude
C_field_pump = 1e-3
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.3 – Fermion Spectra + Visible Majorana")
...
while t < t_max and step < MAX_STEPS:
    ...
    genie_phi_new = cp.clip(genie_phi_new, -0.8, 0.8)
    ...
    C_field = cp.clip(C_field, 0.0, 8.0)
    ...
    if step % 200 == 0:
        ...
        print(f"Step {step:4d} | t={float(t):.3e} | "
              ...
              f"Dirac={history['ferm_dirac_mean'][-1]:.2e} | "
              f"Majorana={history['ferm_majorana_mean'][-1]:.2e} ({maj_frac:.1%}) | "
...
# =============================================================================
# POST-PROCESSING: 43 Hz + FERMION SPECTRA ANALYSIS
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.3 – 43 Hz + FERMION SPATIAL SPECTRA ANALYSIS")
print("="*80)

t_np = np.array(time_history)

# Recompute final fermion fields
...
if len(genie_center_history) > 200:
    ...
    window = slice(max(0, idx_43-25), min(len(freq), idx_43+26))
    peak_idx_g = window.start + np.argmax(power_genie[window])
    peak_freq_g = freq[peak_idx_g]
    peak_power_g = power_genie[peak_idx_g]

    print(f"Simulated time span : {t_np[-1]:.3f}")
    print(f"Frequency resolution: {freq[1]:.4f} Hz")
    print(f"Peak frequency (Genie center): {peak_freq_g:.4f} Hz")
    print(f"Power at peak              : {peak_power_g:.2e}")
    print(f"Power at exact 43 Hz       : {power_genie[idx_43]:.2e}")

    verdict = "❌ no clear 43 Hz"
    if abs(peak_freq_g - 43.0) < 0.7:
        verdict = "✅ STRONG 43 Hz LOCK"
    elif abs(peak_freq_g - 43.0) < 2.5:
    ...
    plt.title(f'43 Hz Resonance Detection – {verdict}', color='white')
    plt.legend()
    plt.grid(alpha=0.25, which='both', color='gray')
    plt.savefig('plots/plot43hz_detection_v13.png', dpi=220, facecolor='black')
    plt.close()
    print("Saved: plots/plot43hz_detection_v13.png")
...
dom_k_mass = k[np.argmax(power_mass[1:]) + 1]
...
plt.ylabel('Power')
plt.title('Fermion Mass Spatial Power Spectrum – ALADIN v1.3')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum.png")
...
plt.xlabel('Wavenumber k (1/a)')
plt.ylabel('Power')
plt.title('Fermion Density Spatial Power Spectrum – ALADIN v1.3')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot44_fermion_density_spatial_spectrum.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot44_fermion_density_spatial_spectrum.png")

# ─── EXISTING FERMION PLOTS (summary – add your previous blocks here if needed) ───
print("\nFermion plots generated (plot01, plot02, plot14, plot39–42, plot43, plot44)")
print("Check especially plot43 & plot44 for spatial structure in fermion fields.")

print("\nALADIN v1.3 complete with fermion spectra!")
print("→ If sausage peak appears in plot43/44 → Z-pinch instability couples to fermions!")
print("→ Share final genie / Majorana / dominant k / 43 Hz verdict for next tuning step.")
