...
print("🌌 ALADIN Plasma Cosmology v39.2 — FULL FUSED RAWKERNEL + PROPER DEDNER CLEANING")
...
# Dedner parameters
c_h_factor = 6.0
kappa_factor = 8.0   # Strong damping
...
# ====================== FULL FUSED RAWKERNEL WITH DEDNER ======================
kernel_code = r'''
extern "C" __global__
void fused_ct_dedner(float* vx, float* vy, float* vz,
                     ...
                     float dt, float dx, int N, float c_h, float kappa)
{
    ...
    if (i < N-1 && j < N-1 && k < N-1) {
        ...
        float Ez_val = 0.03f * (-(vx_avg * By_avg - vy_avg * Bx_avg));   // Gentle strength

        int bz_idx = (i*N + j)*(N+1) + k;
        Bz[bz_idx] += (dt / dx) * Ez_val;
    }

    // Proper Dedner damping
    if (i < N && j < N && k < N) {
        int g = (i*N + j)*N + k;
...
kernel = cp.RawKernel(kernel_code, 'fused_ct_dedner')
...
print("Starting v39.2 with Proper Dedner Cleaning...")
...
for step in range(steps):
    ...
    # Safety clamps
    Bx = cp.nan_to_num(Bx, nan=0.0, posinf=B0, neginf=-B0)
    By = cp.nan_to_num(By, nan=0.0, posinf=B0, neginf=-B0)
    Bz = cp.nan_to_num(Bz, nan=0.0, posinf=B0, neginf=-B0)
...
print("\n✅ v39.2 Finished with Proper Dedner Divergence Cleaning!")
