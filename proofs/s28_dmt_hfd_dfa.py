...
B='data/raw_bdf/S28_DMT.bdf';C='Fz';T=41.0;W=30;D='plots'
os.makedirs(D,exist_ok=True)

try:
 if not os.path.exists(B):raise FileNotFoundError("File missing!")
 ...
 if s<0 or e>len(d):raise ValueError("Window too large")
 pre=d[s:int(T*sf)];post=d[int(T*sf):e]
 if len(pre)<1000 or len(post)<1000:raise ValueError("Segment too short")
 ...
 plt.title(f'S28 DMT p_HFD={ph:.3e} p_DFA={pd:.3e}')
 ...
 print(f"Error: {str(e)}")
 print("Fix: Check file path, channel, or increase WIN.")
