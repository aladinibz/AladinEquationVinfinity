...
# ================== CONFIG ==================
CONFIG = {
    'n_sur': 200,          # Full 200 surrogates as you asked
    'step_sec': 4,
    'window_sec': 5,
    'fs': 128,
...
signal_mse = signal[::16]   # 8 Hz — perfect for entropy
...
# 22. Permutation Entropy timecourse (fixed - no tau argument)
pe_time = []
for i in range(len(time_sec)):
    s = i * step_samples
    e = s + win_samples
    if e > len(signal): break
    w = signal[s:e]
    pe_time.append(permutation_entropy(w, dx=3))   # tau removed - default is 1
plt.figure(figsize=(10,5))
