...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.3 – 15,000 steps – let's fucking go             ║
# ║  (128³ grid, aggressive dt ramp, visible Majorana, 43 Hz + spectra)        ║
# ╚════════════════════════════════════════════════════════════════════════════╝

TEST_MODE = True

GRID_SIZE = 128
MAX_STEPS = 15000          # fuck it — 15k steps
dt_max    = 1e-5           # allow up to 10 µs
CFL       = 0.40           # push a little harder

print("🚀 ALADIN v1.3 – 15,000 steps on 128³ – expect ~12–20 min runtime")
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.3 – 15k steps – fuck it mode")
...
while t < t_max and step < MAX_STEPS:
    ...
    # Aggressive early ramp-up: push dt hard for first 6000 steps
    if step < 6000 and t < 0.01:
        dt_new = cp.maximum(dt_new, 8e-5)          # minimum 80 µs early
        if t < 0.001:
            dt_new = cp.minimum(dt_new * 1.5, 1.5e-4)  # exponential boost

    dt = 0.7 * dt_prev + 0.3 * dt_new
    dt = cp.minimum(dt, dt_max)
    dt = cp.maximum(dt, 5e-8)
    ...
    if current_amp > 0.6:
        local_damping *= 2.0  # emergency damping

    genie_phi_new *= cp.exp(-local_damping * dt)
    genie_phi_new -= 0.008 * cyl_laplacian(genie_phi_new) * dt
    genie_phi_new = cp.clip(genie_phi_new, -1.0, 1.0)  # widened a bit
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.3 – 15,000 steps – FINAL ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    peak_idx_g = window.start + np.argmax(power_genie[window]) if len(window) > 0 else 0
    peak_freq_g = freq[peak_idx_g] if len(freq) > peak_idx_g else 0
    peak_power_g = power_genie[peak_idx_g] if len(power_genie) > peak_idx_g else 0
    ...
    plt.title(f'43 Hz Resonance Detection – 15000 steps – {verdict}', color='white')
    plt.legend()
    plt.grid(alpha=0.25, which='both', color='gray')
    plt.savefig('plots/plot43hz_detection_v13_15000steps.png', dpi=220, facecolor='black')
    plt.close()
    print("Saved: plots/plot43hz_detection_v13_15000steps.png")
...
plt.ylabel('Power')
plt.title('Fermion Mass Spatial Power Spectrum – 15000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_15000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_15000.png")
...
plt.xlabel('Wavenumber k (1/a)')
plt.ylabel('Power')
plt.title('Fermion Density Spatial Power Spectrum – 15000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot44_fermion_density_spatial_spectrum_15000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot44_fermion_density_spatial_spectrum_15000.png")

# Quick time check warning
if t_np[-1] < 0.001:
    print("WARNING: Simulated time still very small (<0.001) — dt ramp-up not enough. Consider forcing larger dt_max or lower grid.")

print("\nALADIN v1.3 – 15,000 steps finished!")
print("Drop the final time span, 43 Hz verdict, dominant k, and any plot observations.")
