...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.7 – 100,000 steps – ZERO DIFFUSION MAX          ║
# ║  (128³ grid, ZERO diffusion, max backreaction, max kink/sausage seeds)     ║
# ╚════════════════════════════════════════════════════════════════════════════╝

GRID_SIZE = 128
MAX_STEPS = 100000         # 100k test – quick but deep enough
dt_max    = 1.2e-4
CFL       = 0.75

print("🚀 ALADIN v1.7 – 100k steps – ZERO DIFFUSION MAX TEST")
print("ZERO diffusion + max backreaction + max kink/sausage seeds – spatial modes should explode!")
...
# =============================================================================
# PARAMETERS – v1.7 nuclear spatial push
# =============================================================================
...
k_sausage = cp.float64(0.7)  # repo-tuned max growth
...
# Initial conditions – MAX SAUSAGE + MAX KINK SEED
...
rho += 1.0 * rho0 * cp.cos(k_sausage * Z)  # max sausage beads
v_phi += 0.8 * vA * cp.sin(Phi) * cp.cos(k_sausage * Z)  # MAX m=1 kink seed
v_r += 0.4 * vA * cp.cos(Phi) * cp.sin(k_sausage * Z)   # extra m=1 radial kick
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.6 – 100k test – ZERO DIFFUSION MAX")
...
while t < t_max and step < MAX_STEPS:
    ...
    # Fermion + Majorana – MAX BACKREACTION
    ...
    majorana_backreaction = 5e-4 * ferm_total_mass  # MAX – fermions push plasma hard
    ...
    genie_phi_new -= 0.0 * cyl_laplacian(genie_phi_new) * dt  # ZERO diffusion – full mode growth
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.6 – 100,000 steps – FINAL ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    plt.grid(alpha=0.3)
    plt.legend()
    plt.savefig('plots/plot35_filtered_43hz_signals_100k_test.png', dpi=200)
    plt.close()
    print("Saved: plots/plot35_filtered_43hz_signals_100k_test.png")
...
plt.ylabel('Power')
plt.title('Fermion Mass z-Spatial Power Spectrum – 100k test')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_100k_test.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_100k_test.png")
...
plt.ylabel('Power')
plt.title('Azimuthal Power Spectrum – Kink Detection (100k test)')
plt.axvline(1, color='red', ls='--', label='m=1 kink')
plt.legend()
plt.grid(alpha=0.3)
plt.savefig('plots/plot45_azimuthal_power_kink_100k_test.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot45_azimuthal_power_kink_100k_test.png")

print("\nALADIN v1.6 – 100,000 steps test finished!")
print("ZERO diffusion + max seeds + strong backreaction active – check sausage & m=1 power!")
