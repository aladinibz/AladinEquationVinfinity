...
print("🌌 Plasma Cosmology v7.0 — 128³ A100 GPU + Fixed Rotation + Conservation")
...
max_v = 300000.0
alpha0 = 0.0010
nu_visc = 0.08          # reduced for better rotation support
...
def run_simulation(use_dm, use_cr=True):
    ...
    print("Inner (0-5 kpc) avg v = {:.1f} km/s".format(np.mean(v_rot[:10])))
    print("Mid (5-15 kpc) avg v = {:.1f} km/s".format(np.mean(v_rot[10:30])))
    print("Outer (15-30 kpc) avg v = {:.1f} km/s".format(np.mean(v_rot[30:])))
...
plt.tight_layout()
plt.show()
plt.savefig('rotation_curves_v7.0.png')

print("✅ v7.0 complete! The figure should now appear clearly. Download 'rotation_curves_v7.0.png' if needed.")
