...
plt.title("ALADIN ∞ ℂ(t) — DFA Scaling Exponent Drop\nEgo Persistence → Random at 43 Hz",fontsize=18)
plt.xlabel("Time [s]",fontsize=14); plt.ylabel("DFA α",fontsize=14)
plt.ylim(0.4,1.0); plt.legend(fontsize=12)
plt.grid(alpha=0.4); plt.tight_layout()
plt.savefig("dfa_scaling_exponent_drop.png",dpi=400)
plt.show()
