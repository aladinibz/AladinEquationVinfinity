"""
ALADIN v1.1.1 — Improved Constrained Transport (CT) + MHD Core
128³ Locked
"""
...
print("🚀 ALADIN v1.1.1 — Improved Full Constrained Transport\n")
...
for J0 in J0_list:
    for rho0 in rho0_list:
        ...
        # Staggered magnetic fields for CT
        ...
        # Initial B_theta (cell-centered then staggered)
        B_theta_cc = (mu0 * J0 * R / 2).astype(np.float32) * np.exp(-(R/(0.6*r_max))**2)
        for i in range(nr):
            B_theta[i, :, :] = B_theta_cc[i]
        ...
        for step in tqdm(range(n_steps), leave=False, desc=f"J0={J0:.1e}"):
            ...
            # Cell-centered fields for calculations
            ...
            # === CONSTRAINED TRANSPORT (CT) EMFs ===
            # Electric fields at cell edges
            ...
            # Resistive diffusion (applied on cell-centered for simplicity)
            B_theta_cc += eta * dt * np.gradient(np.gradient(B_theta_cc, axis=0), axis=0)/dr**2

            # Continuity + Pair + Momentum + Energy
...
plt.title('ALADIN v1.1.1 — Full CT + 128³')
plt.legend()
plt.grid(alpha=0.3)
plt.show()
