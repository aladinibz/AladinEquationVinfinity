...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.5 – 400,000 steps – SAUSAGE AWAKENING           ║
# ║  (128³ grid, ultra-weak diffusion, strong spatial seed, 43 Hz lock)        ║
# ╚════════════════════════════════════════════════════════════════════════════╝

GRID_SIZE = 128
MAX_STEPS = 400000         # 400k steps – spatial instability push
dt_max    = 1e-4           # 100 µs ceiling
CFL       = 0.70           # very aggressive

print("🚀 ALADIN v1.5 – 400,000 steps – SAUSAGE AWAKENING MODE")
print("Ultra-weak diffusion + strong spatial seed – expect sausage power > 0!")
print(f"dt_max = {dt_max:.1e}, CFL = {CFL}, expected runtime ~3–6 hours on A100")
...
# =============================================================================
# PARAMETERS – v1.5 tuned for sausage growth
# =============================================================================
...
# Initial conditions – STRONGER SAUSAGE SEED
...
rho += 0.5 * rho0 * cp.cos(k_sausage * Z)  # 5× stronger initial beads
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.5 – 250k steps – SAUSAGE AWAKENING")
...
while t < t_max and step < MAX_STEPS:
    ...
    dt = cp.maximum(dt, 5e-6)  # hard floor
    ...
    # Genie scalar + STRONG EXPLICIT 43 Hz PUMP + STRONGER SPATIAL SEED
    source_genie = g_genie * J_z + majorana_backreaction
    source_genie += 0.15 * cp.sin(2 * cp.pi * 43 * t) * (1 + 0.4 * cp.cos(k_sausage * Z + 0.5 * t))  # strong pump + spatial seed
    ...
    genie_phi_new -= 0.0005 * cyl_laplacian(genie_phi_new) * dt  # ultra-weak diffusion
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.5 – 250,000 steps – FINAL ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    plt.grid(alpha=0.3)
    plt.legend()
    plt.savefig('plots/plot35_filtered_43hz_signals_250k.png', dpi=200)
    plt.close()
...
plt.ylabel('Power')
plt.title('Fermion Mass Spatial Power Spectrum – 250000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_250k.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_250k.png")
...
plt.xlabel('Wavenumber k (1/a)')
plt.ylabel('Power')
plt.title('Fermion Density Spatial Power Spectrum – 250000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot44_fermion_density_spatial_spectrum_250k.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot44_fermion_density_spatial_spectrum_250k.png")

print("\nALADIN v1.5 – 250,000 steps finished!")
print("Strong spatial seed + ultra-weak diffusion active – check sausage power!")
