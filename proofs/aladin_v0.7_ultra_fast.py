"""
ALADIN v0.7 — ULTRA-OPTIMIZED 3D VERSION (target ~4–7 min for 5000 steps)
All 9 publish plots still generated at the end
"""
...
os.makedirs('plots/aladin_v0.7_ultra_fast', exist_ok=True)
...
# ─── Fields ────────────────────────────────────────────────────────────────
rho = np.ones((nr, ntheta, nz), dtype=np.float32) * rho_0
v_r = np.zeros_like(rho)
v_theta = np.zeros_like(rho)
v_z = np.zeros_like(rho)
p = np.ones_like(rho) * 1e10
e = np.ones_like(rho) * 1e10
T = np.ones_like(rho) * 1e6

B_r = np.zeros_like(rho)
B_theta = np.zeros_like(rho)
B_z = np.zeros_like(rho)

n_pair = np.zeros_like(rho)
...
n_steps = 2000   # test fast — increase later
save_every = 2000  # almost never save maps during loop

k_z = 2 * np.pi / z_max * np.array([1, 2, 4, 8], dtype=np.float32)
mode_amplitudes = np.zeros((len(k_z), n_steps), dtype=np.float32)

S_t = np.zeros(n_steps, dtype=np.float32)
v_A_t = np.zeros(n_steps, dtype=np.float32)
divB_t = np.zeros(n_steps, dtype=np.float32)
E_total_t = np.zeros(n_steps, dtype=np.float32)

Pi_t = np.zeros(n_steps, dtype=np.float32)

delta_Bz_t = np.zeros(n_steps, dtype=np.float32)

E_mag_t = np.zeros(n_steps, dtype=np.float32)
E_kin_t = np.zeros(n_steps, dtype=np.float32)
E_therm_t = np.zeros(n_steps, dtype=np.float32)
E_pair_t = np.zeros(n_steps, dtype=np.float32)

# ─── Time Stepping ─────────────────────────────────────────────────────────
current_dt = dt_max
t_cumulative = np.zeros(n_steps, dtype=np.float32)
t_current = 0.0

for step in tqdm(range(n_steps), desc="v0.7 Ultra Fast 3D", unit="step"):
    try:
        # Adaptive dt
        ...
        # Fast current density (manual FD)
        J_r = (1 / mu0) * (inv_R * (B_z[:,1:,:] - B_z[:,:-1,:]) / dtheta - (B_theta[:,:,1:] - B_theta[:,:,:-1]) / dz)
        J_theta = (1 / mu0) * ((B_r[:,:,1:] - B_r[:,:,:-1]) / dz - (B_z[1:,:,:] - B_z[:-1,:,:]) / dr)
        J_z = (1 / mu0) * (inv_R * ((R_safe[1:,:,:] * B_theta[1:,:,:] - R_safe[:-1,:,:] * B_theta[:-1,:,:]) / dr) - 
                            inv_R * (B_r[:,1:,:] - B_r[:,:-1,:]) / dtheta)
        ...
        # Pair production
        vxB_y = v_theta * B_z - v_z * B_theta
        vxB_z = v_z * B_r - v_r * B_z
        vxB_x = v_r * B_theta - v_theta * B_r
        E_local = np.sqrt(vxB_x**2 + vxB_y**2 + vxB_z**2) + eta * np.sqrt(J_r**2 + J_theta**2 + J_z**2)
        ...
        dP_q_dtheta = inv_R * np.gradient(P_q / c**2 + rho_pair, axis=1) / dtheta
        ...
        dp_dtheta = inv_R * np.gradient(p, axis=1) / dtheta
        ...
        # Resistive diffusion + Hall (vectorized)
        laplacian = lambda f: (np.gradient(np.gradient(f, axis=0), axis=0) / dr**2 + 
                               inv_R * np.gradient(f, axis=0) / dr + 
                               (1/R**2) * np.gradient(np.gradient(f, axis=1), axis=1) / dtheta**2 + 
                               np.gradient(np.gradient(f, axis=2), axis=2) / dz**2)

        B_r += eta * laplacian(B_r) * current_dt
        B_theta += eta * laplacian(B_theta) * current_dt
        B_z += eta * laplacian(B_z) * current_dt
        ...
        flux_z = v_z * B_theta - v_theta * B_r
...
ax.legend()
ax.grid(alpha=0.3)
ax.set_title('Multi-k_z Mode Growth vs Ideal Limits')
plt.tight_layout()
...
sheet_thickness = 0.1 * z_max
v_A_sheet = np.mean(v_A_t)
gamma_tear_analytic = 0.5 * (eta / sheet_thickness**2)**0.5 * (v_A_sheet / sheet_thickness)
...
end_total = time.time()
print("v0.7 ULTRA-OPTIMIZED run complete — 9 plots generated!")
print("Saved in plots/aladin_v0.7_optimized")
print("Total simulated time:", t_current)
print("Runtime (seconds):", round(end_total - start_total, 1))
print("Ready to publish v0.7 3D — let’s goooo!")
