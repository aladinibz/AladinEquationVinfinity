...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.6 – 500,000 steps – FULL SPATIAL PUSH           ║
# ║  (128³ grid, ultra-weak diffusion, strong sausage/kink seeds, sharp lock)  ║
# ╚════════════════════════════════════════════════════════════════════════════╝

GRID_SIZE = 128
MAX_STEPS = 500000         # 500k steps – deep spatial test
dt_max    = 1.2e-4         # 120 µs ceiling
CFL       = 0.75           # aggressive accumulation

print("🚀 ALADIN v1.6 – 500,000 steps – FULL SPATIAL PUSH")
print("Ultra-weak diffusion + strong sausage/kink seeds – expect power at k≈0.7 & m=1!")
...
# =============================================================================
# PARAMETERS – v1.6 tuned for sausage/kink growth
# =============================================================================
...
y_ferm = 1e-3
g_genie = 8e-4
kg_damping = 0.08          # Q ≈ 3375 – ultra-sharp
C_field_pump = 1e-3
...
# Initial conditions – MAXIMUM SAUSAGE + KINK SEED
...
rho += 1.0 * rho0 * cp.cos(k_sausage * Z)  # maximum sausage beads
v_phi += 0.4 * vA * cp.sin(Phi) * cp.cos(k_sausage * Z)  # strong m=1 kink seed
v_r += 0.2 * vA * cp.cos(Phi) * cp.sin(k_sausage * Z)   # extra m=1 radial kick
...
while t < t_max and step < MAX_STEPS:
    ...
    # Genie scalar + STRONG PUMP + SPATIAL SEED
...
phi_mean = cp.mean(ferm_total_final, axis=(0,2)).get()  # mean over r & z
