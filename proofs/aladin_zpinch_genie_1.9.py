...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.9 – 1,000,000 steps – SYMMETRY BREAKER         ║
# ║  (128³ grid, Bessel J₀(kr) + multi-mode kink seed, zero diffusion)         ║
# ╚════════════════════════════════════════════════════════════════════════════╝

GRID_SIZE = 128
MAX_STEPS = 1000000        # 1 MILLION steps – overnight run
dt_max    = 1.2e-4
CFL       = 0.75
...
# Histories – downsample to every 100 steps to save memory
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.9 – 1M steps – SYMMETRY BREAKER")
...
while t < t_max and step < MAX_STEPS:
    ...
    if step % 5000 == 0:
        ...
        print(f"Step {step:7d} | t = {float(t):.6f} | dt = {float(dt):.2e} | "
