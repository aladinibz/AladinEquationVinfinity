...
print("🌌 Plasma Cosmology v6.0 — STABLE 128³ Full 3D")
...
def run_simulation(use_dm, use_cr=True):
    ...
    for k in range(N+1):
        ...
        Bz[:,:,k] = 8e-11 * np.exp(-r2d**2 / 250.0) * np.exp(-zf**2 / 40.0)
    
    # Start almost static (no equilibrium formula yet)
    # We will let forces build the rotation naturally
    p_th = 2e-12 * rho
    ...
    for step in range(steps):
        ...
        # Hard acceleration limiter + update
        acc_x = Fx / (rho + 1e-30)
        acc_y = Fy / (rho + 1e-30)
        acc_z = Fz / (rho + 1e-30)
        acc_x = np.clip(acc_x, -5e7, 5e7)
        acc_y = np.clip(acc_y, -5e7, 5e7)
        acc_z = np.clip(acc_z, -5e7, 5e7)
        ...
        rho = np.nan_to_num(rho, nan=1e-25)
...
print("✅ v6.0 STABLE complete! Share the final Bmax, vmax, drift % and curve shapes.")
