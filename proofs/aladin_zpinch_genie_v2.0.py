...
GRID_SIZE = 128
MAX_STEPS = 1000           # short magic test – quick check
dt_max    = 1.2e-4
CFL       = 0.75

print("🚀 ALADIN v1.6 – 1000 steps MAGIC TEST – BESSEL + LIGHT m=1 KINK")
print("Bessel J₀(kr) + ultra-mild m=1 kink with phase twist – let's see if symmetry break wakes sausage!")
...
# Bessel J0(kr) radial profile – repo eigenmode shape (kept from last successful run)
r_1d = r.get()
...
# Light m=1 kink seed with phase twist – ultra-mild + smooth spiral (friend's advice)
rho += 0.01 * rho0 * cp.cos(Phi + 0.5 * k_sausage * Z)  # phase twist Phi + 0.5 kz
v_phi += 0.005 * vA * cp.sin(Phi + 0.5 * k_sausage * Z)
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.6 – 1000 steps MAGIC TEST")
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.6 – 1000 steps MAGIC TEST – FINAL ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 33:
    ...
    plt.title('Clean 43 Hz Bandpass Filtered Signals – 1000 steps MAGIC TEST')
    plt.grid(alpha=0.3)
    plt.legend()
    plt.savefig('plots/plot35_filtered_43hz_signals_1000magic.png', dpi=200)
    plt.close()
    print("Saved: plots/plot35_filtered_43hz_signals_1000magic.png")
else:
...
plt.ylabel('Power')
plt.title('Fermion Mass z-Spatial Power Spectrum – 1000 steps MAGIC TEST')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_1000magic.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_1000magic.png")
...
plt.ylabel('Power')
plt.title('Azimuthal Power Spectrum – Kink Detection (1000 steps MAGIC TEST)')
plt.axvline(1, color='red', ls='--', label='m=1 kink')
plt.legend()
plt.grid(alpha=0.3)
plt.savefig('plots/plot45_azimuthal_power_kink_1000magic.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot45_azimuthal_power_kink_1000magic.png")

print("\nALADIN v1.6 – 1000 steps MAGIC TEST finished!")
print("Bessel + light m=1 kink active – check sausage band and m=1 power!")
