...
GRID_SIZE = 128
MAX_STEPS = 500000         # back to stable 500k
dt_max    = 1.2e-4
CFL       = 0.75

print("🚀 ALADIN v1.9 STABLE – 500k steps – SINGLE STRONG KINK SEED")
print("Bessel J₀(kr) + strong m=3 kink seed + zero diffusion + strong backreaction")
print(f"dt_max = {dt_max:.1e}, CFL = {CFL}, expected runtime ~3–4 hours on A100")
...
# Strong single m=3 kink seed (symmetry breaker, low RAM)
rho += 0.15 * rho0 * cp.cos(3 * Phi + k_sausage * Z)
v_phi += 0.08 * vA * cp.sin(3 * Phi + k_sausage * Z)
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.9 – 500k steps – STABLE")
...
try:
    while t < t_max and step < MAX_STEPS:
        ...
        if step % 5000 == 0:
            ...
            print(f"Step {step:7d} | t = {float(t):.6f} | dt = {float(dt):.2e} | genie = {history['genie_amp'][-1] if history['genie_amp'] else 0:.2e} | "
                  f"Majorana_frac = {maj_frac*100:.1f}% | TotalFerm = {history['ferm_total_mean'][-1] if history['ferm_total_mean'] else 0:.2e} | RAM: {ram_gb:.1f} GB")
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.9 – 500,000 steps – FINAL ANALYSIS")
print("="*80)
...
power_mass = np.abs(np.fft.rfft(ferm_mass_zmean))**2 / len(ferm_mass_zmean) if len(ferm_mass_zmean) > 0 else 0.0
power_dens = np.abs(np.fft.rfft(ferm_dens_zmean))**2 / len(ferm_dens_zmean) if len(ferm_dens_zmean) > 0 else 0.0
...
power_phi = np.abs(np.fft.rfft(phi_mean))**2 / N_GRID if len(phi_mean) > 0 else 0.0
...
print("\nALADIN v1.9 – 500,000 steps finished!")
