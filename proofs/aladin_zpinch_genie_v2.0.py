...
print("🚀 ALADIN v1.6 – 1000 steps TEST – BESSEL + m=1 KINK ONLY")
print("Bessel J₀(kr) radial seed + light m=1 kink – symmetry break test")
...
# Bessel J0(kr) radial profile – repo eigenmode shape
r_1d = r.get()
...
# Only m=1 kink seed – symmetry break with minimal load
rho += 0.05 * rho0 * cp.cos(1 * Phi) * cp.cos(k_sausage * Z)
v_phi += 0.02 * vA * cp.sin(1 * Phi) * cp.sin(k_sausage * Z)
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.6 – 1000 steps TEST – BESSEL + m=1 KINK")
...
print("Bessel seed + m=1 kink active – check sausage band and m=1 power!")
