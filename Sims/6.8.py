...
print("🌌 Plasma Cosmology v6.8 — 128³ A100 GPU + Fixed Conservation + CR Diffusion + Cooling")
...
def run_simulation(use_dm, use_cr=True):
    ...
    for step in range(steps):
        ...
        if step % 50 == 0 or step == steps-1:
            ...
            print(f"  rho min/max = {cp.min(rho):.2e}/{cp.max(rho):.2e} | T min/max = {cp.min(T):.1f}/{cp.max(T):.1f} K")
...
print("✅ v6.8 complete! Share the rotation curve shapes, Bmax/vmax, energy drifts, and any early min/max diagnostics.")
