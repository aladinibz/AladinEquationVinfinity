# mega ultra maniac v1.0.2 – ULTIMATE BOSS MANIAC VERSION
# 3D Resistive MHD Z-pinch Kink POC (128³) with TRUE Yee CT (resistivity in EMF) + Spitzer + Bremsstrahlung w/ Gaunt
# Line-tied BCs, all 13 plots fully integrated, tqdm progress, 400 dpi, 3D viz – ready for Zenodo
...
# ────────────────────────────────────────────────
@jit
def apply_line_tied_bc(state):
    state['S'] = state['S'].at[2, :, :, 0].set(0.0)
    state['S'] = state['S'].at[2, :, :, -1].set(0.0)

    state['B'] = state['B'].at[0, :, :, 0].set(state['B'][0, :, :, 1])
    state['B'] = state['B'].at[0, :, :, -1].set(state['B'][0, :, :, -2])
    state['B'] = state['B'].at[1, :, :, 0].set(state['B'][1, :, :, 1])
    state['B'] = state['B'].at[1, :, :, -1].set(state['B'][1, :, :, -2])

    state['B'] = state['B'].at[2, :, :, 0].set(2 * state['B'][2, :, :, 1] - state['B'][2, :, :, 2])
    state['B'] = state['B'].at[2, :, :, -1].set(2 * state['B'][2, :, :, -2] - state['B'][2, :, :, -3])

    state['rho'] = state['rho'].at[:, :, 0].set(state['rho'][:, :, 1])
    state['rho'] = state['rho'].at[:, :, -1].set(state['rho'][:, :, -2])
    state['p'] = state['p'].at[:, :, 0].set(state['p'][:, :, 1])
    state['p'] = state['p'].at[:, :, -1].set(state['p'][:, :, -2])
...
def step(state, dx):
    ...
    # Conservative advection fluxes – FIXED broadcasting
    p_mag = jnp.sum(B**2, axis=0) / (2 * CONFIG['mu0'])
    p_tot = p + p_mag

    v_outer = v[:, None, ...] * v[None, ...]  # (3,3,Nx,Ny,Nz)
    B_outer = B[:, None, ...] * B[None, ...]  # (3,3,Nx,Ny,Nz)
    f_S = rho * v_outer + p_tot[None, None, ...] * jnp.eye(3)[:, :, None, None, None] - B_outer / CONFIG['mu0']

    f_rho = rho * v  # (3,Nx,Ny,Nz)
    f_tau = (tau + p + p_mag) * v  # (3,Nx,Ny,Nz)
    f_B = v[:, None, ...] * B[None, ...] - B[:, None, ...] * v[None, ...]  # (3,3,Nx,Ny,Nz)

    # Divergences
    drho_dt = -sum(jnp.gradient(f_rho[i], axis=i) for i in range(3)) / dx
    dS_dt += -jnp.stack([jnp.gradient(f_S[i,j], axis=j) for i in range(3) for j in range(3)], axis=0).reshape(3, *rho.shape) / dx
    dtau_dt = -sum(jnp.gradient(f_tau[i], axis=i) for i in range(3)) / dx
    dB_dt = -jnp.stack([jnp.gradient(f_B[i,j], axis=j) for i in range(3) for j in range(3)], axis=0).reshape(3, *rho.shape) / dx
    ...
    # Heating and cooling rates (for plots)
...
for step_num in tqdm(range(CONFIG['steps']), desc="Progress"):
    ...
    if step_num % 20 == 0:
        print(f"Step {step_num} | t = {real_time_history[-1]:.2e} s | r = {state['pinch_radius']:.4e} | imbalance = {state['imbalance_ratio']:.4e} | heating = {state['heating_rate']:.4e} | cooling = {state['cooling_rate']:.4e} | |J| = {state['mean_j_mag']:.4e}")
...
for m in range(4):
    ...
    if not np.isnan(gamma_measured[m]):
        fit_curve = exp_growth(time_fft, *curve_fit(exp_growth, time_fft[:30], m_amplitudes[:30, m], p0=[1e-6, 1e7])[0])
        ax.semilogy(time_fft, fit_curve, '--', color=colors[m], alpha=0.7, lw=2)
...
ax = fig.gca()
ax.set_facecolor('black')
ax.plot(real_time, energy_ratio, lw=3, color='#00FFFF', label='Total / E(0)')
ax.axhline(1.0, color='gray', ls='--')
ax.set_xlabel('Time [μs]', color='white')
ax.set_ylabel('Energy ratio', color='white')
...
ax = fig.gca()
ax.set_facecolor('black')
ax.plot(real_time[:-1], diagnostics['dt_used'], lw=3, color='#FFD700', label='Adaptive dt')
ax.axhline(CONFIG['dt_max'], color='gray', ls='--')
ax.set_xlabel('Time [μs]', color='white')
ax.set_ylabel('Timestep dt [s]', color='white')
ax.set_title('Adaptive Timestep History', color='white')
ax.set_yscale('log')
ax.legend(frameon=False, labelcolor='white')
ax.grid(alpha=0.2, color='gray')
ax.tick_params(colors='white')
...
ax = fig.gca()
ax.set_facecolor('black')
ax.plot(real_time[:-1], diagnostics['pinch_radius'], lw=3, color='#00FF00', label='⟨r⟩')
ax.set_xlabel('Time [μs]', color='white')
...
ax.set_title('Force Balance (Lorentz vs Pressure)', color='white')
ax.set_yscale('log')
ax.legend(frameon=False, labelcolor='white')
ax.grid(alpha=0.2, color='gray')
ax.tick_params(colors='white')
...
ax = fig.gca()
ax.set_facecolor('black')
ax.plot(real_time[:-1], diagnostics['imbalance_ratio'], lw=3, color='#FF1493', label='Force imbalance ratio')
...
ax = fig.gca()
ax.set_facecolor('black')
ax.plot(real_time[:-1], diagnostics['mean_j_mag'], lw=3, color='#FF69B4', label='Mean |J|')
ax.set_xlabel('Time [μs]', color='white')
...
ax = fig.gca()
ax.set_facecolor('black')
ax.plot(real_time[:-1], diagnostics['cooling_rate'], lw=3, color='#00CED1', label='Bremsstrahlung cooling')
ax.set_xlabel('Time [μs]', color='white')
ax.set_ylabel('Rate [W/m³]', color='white')
