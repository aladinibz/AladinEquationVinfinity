...
print("🌌 Plasma Cosmology v10.9 — FULL COMPLETE CODE (histogram shape fixed)")
...
def run_simulation(use_dm, use_cr=True):
    ...
    for step in range(steps):
        ...
        mx[1:-1,:,:] -= dt/dx * (f_mx[1:,:,:] - f_mx[:-1,:,:])
        my[1:-1,:,:] -= dt/dx * (f_my[1:,:,:] - f_my[:-1,:,:])
        mz[1:-1,:,:] -= dt/dx * (f_mz[1:,:,:] - f_mz[:-1,:,:])
        E_total[1:-1,:,:] -= dt/dx * (f_E[1:,:,:] - f_E[:-1,:,:])
    ...
    # Kinetic energy balance table (shape-safe)
    ...
    # All terms on exact same mid-plane slice
    Jx_mid = (cp.gradient(Bz_c, dx, axis=1) - cp.gradient(By_c, dx, axis=2))[:,:,mid]
    Jy_mid = (cp.gradient(Bx_c, dx, axis=2) - cp.gradient(Bz_c, dx, axis=0))[:,:,mid]
    Jz_mid = (cp.gradient(By_c, dx, axis=0) - cp.gradient(Bx_c, dx, axis=1))[:,:,mid]
    JxB_x_mid = Jy_mid * Bz_c[:,:,mid] - Jz_mid * By_c[:,:,mid]
    JxB_y_mid = Jz_mid * Bx_c[:,:,mid] - Jx_mid * Bz_c[:,:,mid]
    a_JxB_r = ((JxB_x_mid * r_hat_x[:,:,mid] + JxB_y_mid * r_hat_y[:,:,mid]) / (rho[:,:,mid] + 1e-6)).flatten().get()
    
    tension_mid = (Bx_c * cp.gradient(Bx_c, dx, axis=0) + By_c * cp.gradient(By_c, dx, axis=1) + Bz_c * cp.gradient(Bz_c, dx, axis=2))[:,:,mid]
    a_tension_r = (tension_mid / (rho[:,:,mid] + 1e-6)).flatten().get()
    
    P_total_mid = P_total[:,:,mid]
    a_press_r = - ((cp.gradient(P_total_mid, dx, axis=0) * r_hat_x[:,:,mid] + cp.gradient(P_total_mid, dx, axis=1) * r_hat_y[:,:,mid]) / (rho[:,:,mid] + 1e-6)).flatten().get()
    
    a_turb_r = - ((cp.gradient(P_total_mid, dx, axis=0) * r_hat_x[:,:,mid] + cp.gradient(P_total_mid, dx, axis=1) * r_hat_y[:,:,mid]) / (rho[:,:,mid] + 1e-6)).flatten().get()
    ...
    plt.grid(True)
    plt.show()
    plt.savefig('tully_fisher_v10.9.png')
...
print("✅ v10.9 complete! Histogram shape error fixed.")
