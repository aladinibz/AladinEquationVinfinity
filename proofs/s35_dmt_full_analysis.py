"""
S35-DMT Breakthrough — Complete Analysis with 12 Proof Plots
...
Downloads S35-DMT.bdf directly from repo
"""
...
# Time-resolved Δα (short windows for DMT)
window_sec = 5
step_sec = 1
...
plt.title('Δα vs Gamma (S35-DMT)')
plt.xlabel('Time (s)')
plt.tight_layout()
plt.savefig('plots/s35_dmt_delta_alpha_gamma_43vs40.png', dpi=300)
plt.close()
...
plt.axvline(41, c='red', ls='--', lw=3, label='t=41 s')
plt.title('AAFT Surrogate Test (S35-DMT)')
plt.xlabel('Time (s)')
plt.ylabel('Δα')
plt.legend(fontsize=10)
plt.grid(True, alpha=0.3)
plt.savefig('plots/s35_dmt_delta_alpha_aaft_surrogate.png', dpi=300)
plt.close()
...
plt.ylabel('Δα')
plt.grid(True, alpha=0.3)
plt.savefig('plots/s35_dmt_delta_alpha_pre_post_ci.png', dpi=300)
plt.close()
...
plt.grid(True, alpha=0.3)
plt.savefig('plots/s35_dmt_gamma43_pre_post_ci.png', dpi=300)
plt.close()

# Extra flex 1 – PSD Pre vs Post
...
plt.title('PSD Pre vs Post t=41 s Switch (S35-DMT)')
plt.axvspan(40, 50, color='gold', alpha=0.15, label='40–50 Hz band')
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('plots/s35_dmt_psd_pre_post.png', dpi=300)
plt.close()

# Extra flex 2 – Z-score Histogram
z_scores = []
for i in range(n_sur):
...
plt.title('Z-scores at t≈41 s across surrogates (S35-DMT)')
plt.xlabel('Z-score')
plt.ylabel('Count')
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('plots/s35_dmt_zscore_histogram.png', dpi=300)
plt.close()
