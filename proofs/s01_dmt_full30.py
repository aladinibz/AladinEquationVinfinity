...
# ================== CONFIG ==================
CONFIG = {
    'n_sur': 50,           # Safe for Colab free tier (change to 100 later)
    'step_sec': 4,
    'window_sec': 5,
    'fs': 128,
...
print(f"Surrogates: {CONFIG['n_sur']} | Step: {CONFIG['step_sec']}s (optimized for Colab)")
...
# 17. True Multiscale Sample Entropy (optimized for Colab RAM)
...
def multiscale_sample_entropy(x, max_scale=12, m=2):
    mse = []
...
# Downsample signal for MSE to save RAM
signal_mse = signal[::4]   # 32 Hz - plenty for entropy
mse = multiscale_sample_entropy(signal_mse, max_scale=12)

plt.figure(figsize=(10,5))
plt.plot(range(1, len(mse)+1), mse, color=COLORS['mse'], lw=3)
plt.xlabel('Scale')
plt.ylabel('Sample Entropy')
plt.title('17. True Multiscale Sample Entropy of LFP (optimized)')
plt.grid(True)
