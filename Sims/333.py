...
print("Exact HLLD + Yee CT + Rankine-Hugoniot + Entropy + Kinetic Energy | N=256 fixed")
...
for step in range(steps):
    ...
    my += dt * rho * g_y
    mz += dt * rho * g_z
    # Conservative energy update (gravity work)
    ...
    # SN momentum kicks (random direction)
    ...
    if step % 50 == 0:
        print(f"Step {step:4d} | Bmax = {float(cp.max(cp.sqrt(Bx_c**2 + By_c**2 + Bz_c**2))):.2f} μG | vmax = {float(cp.max(vtot)):.1f} km/s")
