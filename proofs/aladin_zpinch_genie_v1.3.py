...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.3 – 20,000 steps – full send                    ║
# ║  (128³ grid, strong dt ramp, Majorana domination, 43 Hz + spectra)         ║
# ╚════════════════════════════════════════════════════════════════════════════╝

TEST_MODE = True

GRID_SIZE = 128
MAX_STEPS = 20000          # 20k steps – let's see real evolution
dt_max    = 1e-5           # 10 µs max
CFL       = 0.40           # push it

print("🚀 ALADIN v1.3 – 20,000 steps on 128³ – expect ~15–25 min runtime")
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.3 – 20k steps – full send")
...
while t < t_max and step < MAX_STEPS:
    ...
    # Aggressive ramp-up: first 8000 steps push dt hard
    if step < 8000 and t < 0.02:
        dt_new = cp.maximum(dt_new, 1e-4)          # min 100 µs early
        if t < 0.005:
            dt_new = cp.minimum(dt_new * 1.6, 2e-4)  # strong exponential boost
    ...
    genie_phi_new = cp.clip(genie_phi_new, -1.0, 1.0)
...
if len(genie_center_history) > 200:
    ...
    if window_end > window_start and len(freq) > 0:
        ...
        peak_idx_g = window_start + np.argmax(power_genie[window])
...
# Time warning
if t_np[-1] < 0.02:
    print("WARNING: Time span still < 0.02 s – consider forcing dt_max = 5e-5 or lower grid for test.")

print("\nALADIN v1.3 – 20,000 steps max finished!")
print("Drop the time span, 43 Hz verdict, dominant k, Majorana %, and plot vibes.")
