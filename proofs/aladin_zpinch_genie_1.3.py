...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.3 – 25,000 steps – FULL SEND                     ║
# ║  (128³ grid, strong dt ramp-up, Majorana domination, 43 Hz + spectra)      ║
# ╚════════════════════════════════════════════════════════════════════════════╝

TEST_MODE = True

GRID_SIZE = 128
MAX_STEPS = 25000          # 25k steps – all in
dt_max    = 2e-5           # 20 µs max step size
CFL       = 0.45           # aggressive but monitored

print("🚀 ALADIN v1.3 – 25,000 steps on 128³ – expect ~18–35 min runtime on A100")
...
while t < t_max and step < MAX_STEPS:
    ...
    # Aggressive ramp-up: first 10k steps push dt HARD
    if step < 10000 and t < 0.05:
        dt_new = cp.maximum(dt_new, 2e-4)          # min 200 µs early
        if t < 0.01:
            dt_new = cp.minimum(dt_new * 1.8, 4e-4)  # strong exponential boost
    ...
    genie_phi_new -= 0.01 * cyl_laplacian(genie_phi_new) * dt
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.3 – 25,000 steps – FINAL ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    plt.title(f'43 Hz Resonance Detection – 25000 steps – {verdict}', color='white')
    plt.legend()
    plt.grid(alpha=0.25, which='both', color='gray')
    plt.savefig('plots/plot43hz_detection_v13_25000steps.png', dpi=220, facecolor='black')
    plt.close()
    print("Saved: plots/plot43hz_detection_v13_25000steps.png")
else:
...
plt.ylabel('Power')
plt.title('Fermion Mass Spatial Power Spectrum – 25000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_25000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_25000.png")
...
plt.xlabel('Wavenumber k (1/a)')
plt.ylabel('Power')
plt.title('Fermion Density Spatial Power Spectrum – 25000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot44_fermion_density_spatial_spectrum_25000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot44_fermion_density_spatial_spectrum_25000.png")
...
print("\nALADIN v1.3 – 25,000 steps finished!")
