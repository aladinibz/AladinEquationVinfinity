...
from scipy.special import j0  # for Bessel J0 radial profile

# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.7 – 50,000 steps – BESSEL J₀ SEED TEST         ║
# ║  (128³ grid, Bessel J₀(kr) radial seed, zero diffusion, strong kink)       ║
# ╚════════════════════════════════════════════════════════════════════════════╝

GRID_SIZE = 128
MAX_STEPS = 50000          # quick 50k test with Bessel seed
dt_max    = 1.2e-4
CFL       = 0.75

print("🚀 ALADIN v1.7 – 50k steps – BESSEL J₀ RADIAL SEED TEST")
print("Bessel J₀(kr) eigenmode seed + zero diffusion – sausage should finally grow!")
...
k_sausage = cp.float64(0.7)  # repo-tuned for fastest growth
...
# Initial conditions – BESSEL J₀(kr) RADIAL SEED + STRONG KINK
...
# Bessel J0(kr) radial profile for sausage eigenmode
r_1d = r.get()  # numpy for scipy j0
j0_kr = j0(k_sausage * r_1d)
j0_kr = cp.array(j0_kr)
j0_kr = cp.maximum(j0_kr, 0.0)  # clip negative lobes if wanted
j0_kr /= cp.max(cp.abs(j0_kr))  # normalize max = 1

# 3D seed: J₀(kr) * cos(kz)
rho += 1.0 * rho0 * cp.outer(j0_kr, cp.ones((N_GRID, N_GRID))) * cp.cos(k_sausage * Z)
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.7 – 50k test – BESSEL J₀ SEED")
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.7 – 50,000 steps – FINAL ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    plt.grid(alpha=0.3)
    plt.legend()
    plt.savefig('plots/plot35_filtered_43hz_signals_50k_bessel.png', dpi=200)
    plt.close()
    print("Saved: plots/plot35_filtered_43hz_signals_50k_bessel.png")
...
plt.ylabel('Power')
plt.title('Fermion Mass z-Spatial Power Spectrum – 50k Bessel test')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_50k_bessel.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_50k_bessel.png")
...
plt.ylabel('Power')
plt.title('Azimuthal Power Spectrum – Kink Detection (50k Bessel test)')
plt.axvline(1, color='red', ls='--', label='m=1 kink')
plt.legend()
plt.grid(alpha=0.3)
plt.savefig('plots/plot45_azimuthal_power_kink_50k_bessel.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot45_azimuthal_power_kink_50k_bessel.png")

print("\nALADIN v1.7 – 50,000 steps test finished!")
print("Bessel J₀(kr) radial seed + zero diffusion active – check sausage power!")
