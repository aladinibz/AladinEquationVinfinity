...
print("🌌 Plasma Cosmology v6.2 — 128³ CuPy + FULL RUNTIME ERROR HANDLING")

try:
    # GPU check
    if cp.cuda.runtime.getDeviceCount() == 0:
        raise RuntimeError("No GPU detected. Please go to Runtime → Change runtime type → GPU")

    # Memory pool
    pool = cp.get_default_memory_pool()
    pool.set_limit(size=5 * 1024**3)  # 5 GB safe for Colab
    print(f"✅ CuPy GPU ready | Memory pool limit: 5 GB")
    ...
    def run_simulation(use_dm, use_cr=True):
        ...
        for step in range(steps):
            ...
            # Extra safety
            vx = cp.nan_to_num(vx, nan=0.0)
            vy = cp.nan_to_num(vy, nan=0.0)
            vz = cp.nan_to_num(vz, nan=0.0)
            rho = cp.nan_to_num(rho, nan=1e-25)
    ...
    print("❌ CUDA Runtime Error (driver issue):")
    print(e)
    print("\nFix: Runtime → Change runtime type → GPU → Restart session → Run the pip install cell again.")
except Exception as e:
    print("❌ Unexpected error:")
    print(e)
    print("\nTry restarting the runtime and re-running the pip install cell.")

print("Code with error handling finished.")
