"""
ALADIN Plasma Stability Law v0.7 — FULL 3D FIXED & PUBLISH-READY (9 PLOTS)
All undefined variables fixed (a_initial, e, T, etc.)
Full 3D cylindrical resistive QMHD + QED pair plasma effects
"""
...
mu0 = 4 * np.pi * 1e-7
c = 3e8
a_initial = 1.0  # FIXED: initial radius scale for ideal mode comparison
rho_0 = 1.745e12
gamma = 5/3
eta = 1e-5
eta_divB = 0.2
...
e = np.ones_like(R, dtype=np.float32) * 1e10  # internal energy
T = np.ones_like(R, dtype=np.float32) * 1e6   # temperature
...
n_steps = 1000  # small for test — increase to 5000/10000 later
save_every = 200
...
for step in tqdm(range(n_steps), desc="v0.7 Full 3D Run", unit="step"):
    try:
