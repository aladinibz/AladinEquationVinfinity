...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.3 – 10,000 steps edition                        ║
# ║  (128³ grid, faster dt ramp-up, visible Majorana, 43 Hz + spectra)         ║
# ╚════════════════════════════════════════════════════════════════════════════╝

TEST_MODE = True

GRID_SIZE = 128
MAX_STEPS = 10000          # ← your request: 10,000 steps
dt_max    = 1e-5           # increased to help reach real time
CFL       = 0.35

print("🚀 10,000 steps mode on 128³ – expect ~8–15 min runtime on A100")
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.3 – 10,000 steps evolution")
...
while t < t_max and step < MAX_STEPS:
    ...
    # Early boost: force larger dt for first 4000 steps if time is tiny
    if step < 4000 and t < 5e-3:
        dt_new = cp.maximum(dt_new, 5e-5)          # minimum 5e-5 early
        if t < 1e-4:
            dt_new = cp.minimum(dt_new * 1.4, 8e-5)  # exponential push

    dt = 0.6 * dt_prev + 0.4 * dt_new
    dt = cp.minimum(dt, dt_max)
    dt = cp.maximum(dt, 2e-7)                      # raised floor
    ...
    # Genie scalar with emergency damping if too large
    ...
    # Adaptive damping if amplitude gets large
    current_amp = cp.mean(cp.abs(genie_phi_new))
    local_damping = kg_damping
    if current_amp > 0.5:
        local_damping *= 1.5  # extra damping to prevent blow-up
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.3 – 10,000 steps – 43 Hz + FERMION SPATIAL SPECTRA ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    plt.title(f'43 Hz Resonance Detection – 10000 steps – {verdict}', color='white')
    plt.legend()
    plt.grid(alpha=0.25, which='both', color='gray')
    plt.savefig('plots/plot43hz_detection_v13_10000steps.png', dpi=220, facecolor='black')
    plt.close()
    print("Saved: plots/plot43hz_detection_v13_10000steps.png")
...
plt.ylabel('Power')
plt.title('Fermion Mass Spatial Power Spectrum – 10000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_10000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_10000.png")
...
plt.xlabel('Wavenumber k (1/a)')
plt.ylabel('Power')
plt.title('Fermion Density Spatial Power Spectrum – 10000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot44_fermion_density_spatial_spectrum_10000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot44_fermion_density_spatial_spectrum_10000.png")

print("\nALADIN v1.3 – 10,000 steps finished!")
print("Check plots folder for updated spectra and 43 Hz detection.")
print("Especially look at simulated time span, genie growth, and whether spectra show peaks.")
