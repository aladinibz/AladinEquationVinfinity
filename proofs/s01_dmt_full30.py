...
# ================== CONFIG ==================
CONFIG = {
    'n_sur': 100,          # Change to 200 for final high-accuracy run
    'step_sec': 4,         # Change to 2 for max resolution
    'window_sec': 5,
    'fs': 128,
...
print("=== ALADIN EEG VALIDATION SUITE – 30 BEAUTIFUL PLOTS ===")
print(f"Author: {CONFIG['author']}")
print(f"Surrogates: {CONFIG['n_sur']} | Step: {CONFIG['step_sec']}s (optimized)")
...
# True sub-millisecond t_peak with upsampling
target_fs = 2048
...
plt.grid(True)
plt.savefig('plots/s01_dmt_01_fluctuation_functions.png', dpi=600)
print("Plot 01 done: s01_dmt_01_fluctuation_functions.png")
...
plt.grid(True)
plt.savefig('plots/s01_dmt_02_hq_curve.png', dpi=600)
print("Plot 02 done: s01_dmt_02_hq_curve.png")
...
plt.grid(True)
plt.savefig('plots/s01_dmt_03_singularity_spectrum.png', dpi=600)
print("Plot 03 done: s01_dmt_03_singularity_spectrum.png")
...
plt.grid(True)
plt.legend()
plt.savefig('plots/s01_dmt_04_delta_alpha_timecourse.png', dpi=600)
print("Plot 04 done: s01_dmt_04_delta_alpha_timecourse.png")
...
plt.xlabel('Time (s)')
plt.tight_layout()
plt.savefig('plots/s01_dmt_05_delta_alpha_gamma_43vs40.png', dpi=600)
print("Plot 05 done: s01_dmt_05_delta_alpha_gamma_43vs40.png")

# AAFT surrogate on Δα — OPTIMIZED WITH PARALLEL JOBLIB
n_sur = CONFIG['n_sur']
...
print("Computing AAFT surrogates (parallelized)...")
surr_list = Parallel(n_jobs=-1)(delayed(compute_one_surrogate)(i, signal, time_sec, step_samples, win_samples, q) for i in range(n_sur))
...
plt.savefig('plots/s01_dmt_06_delta_alpha_aaft_surrogate.png', dpi=600)
print("Plot 06 done: s01_dmt_06_delta_alpha_aaft_surrogate.png")
...
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('plots/s01_dmt_10_surrogate_zscore_histogram.png', dpi=600)
print("Plot 10 done: s01_dmt_10_surrogate_zscore_histogram.png")
...
plt.ylabel('Δα')
plt.grid(True, alpha=0.3)
plt.savefig('plots/s01_dmt_07_delta_alpha_pre_post_permutation.png', dpi=600)
print("Plot 07 done: s01_dmt_07_delta_alpha_pre_post_permutation.png")
...
plt.grid(True, alpha=0.3)
plt.savefig('plots/s01_dmt_08_gamma43_pre_post_permutation.png', dpi=600)
print("Plot 08 done: s01_dmt_08_gamma43_pre_post_permutation.png")
...
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('plots/s01_dmt_09_psd_pre_post.png', dpi=600)
print("Plot 09 done: s01_dmt_09_psd_pre_post.png")
...
# Modulate with real gamma envelope for "driven by Genie"
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('plots/s01_dmt_11_superradiance_mt_disorder.png', dpi=600)
print("Plot 11 done: s01_dmt_11_superradiance_mt_disorder.png")
...
plt.grid(True)
plt.savefig('plots/s01_dmt_12_alpha_power_vs_time.png', dpi=600)
print("Plot 12 done: s01_dmt_12_alpha_power_vs_time.png")
...
plt.grid(True)
plt.savefig('plots/s01_dmt_13_beta_power_vs_time.png', dpi=600)
print("Plot 13 done: s01_dmt_13_beta_power_vs_time.png")
...
plt.grid(True)
plt.savefig('plots/s01_dmt_14_buzsaki_theta_43hz_pac.png', dpi=600)
print("Plot 14 done: s01_dmt_14_buzsaki_theta_43hz_pac.png")
...
if burst_triggered:
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('plots/s01_dmt_15_buzsaki_gamma_bursts_on_theta.png', dpi=600)
    print("Plot 15 done: s01_dmt_15_buzsaki_gamma_bursts_on_theta.png")

# 16. Multitaper LFP Power Spectrum – 43 Hz Peak
f_mt, pxx_mt = mne.time_frequency.psd_array_multitaper(signal, sfreq=fs, fmin=1, fmax=100, n_jobs=1)
peak_idx = np.argmax(pxx_mt)
peak_freq = f_mt[peak_idx]

plt.figure(figsize=(12,6))
plt.semilogy(f_mt, pxx_mt, color=COLORS['gamma'], lw=3)
...
plt.xlabel('Frequency (Hz)')
plt.ylabel('Power Spectral Density')
plt.title('16. Multitaper LFP Power Spectrum – 43 Hz Peak')
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('plots/s01_dmt_16_multitaper_lfp_power_spectrum.png', dpi=600)
print("Plot 16 done: s01_dmt_16_multitaper_lfp_power_spectrum.png")

# 17. True Multiscale Sample Entropy
def sample_entropy(x, m=2, r=None):
    ...
    B = np.sum(np.all(np.abs(x_m[:, None] - x_m[None, :]) <= r, axis=2), axis=1) - 1
    A = np.sum(np.all(np.abs(x_m1[:, None] - x_m1[None, :]) <= r, axis=2), axis=1) - 1
    return -np.log(np.mean(A / B)) if np.mean(B) > 0 else np.nan

def multiscale_sample_entropy(x, max_scale=20, m=2):
    mse = []
...
mse = multiscale_sample_entropy(signal)
plt.figure(figsize=(10,5))
plt.plot(range(1, len(mse)+1), mse, color=COLORS['mse'], lw=3)
plt.xlabel('Scale')
plt.ylabel('Sample Entropy')
plt.title('17. True Multiscale Sample Entropy of LFP')
plt.grid(True)
plt.savefig('plots/s01_dmt_17_lfp_multiscale_entropy.png', dpi=600)
print("Plot 17 done: s01_dmt_17_lfp_multiscale_entropy.png")
...
plt.grid(True)
plt.savefig('plots/s01_dmt_18_izhi_network_rate.png', dpi=600)
print("Plot 18 done: s01_dmt_18_izhi_network_rate.png")

# 19. MFDFA on Izhikevich Network Output (separate lag)
...
plt.grid(True)
plt.savefig('plots/s01_dmt_19_izhi_mfdfa_output.png', dpi=600)
print("Plot 19 done: s01_dmt_19_izhi_mfdfa_output.png")

# 20. Granger Causality pineal → frontal + ADF check (with FDR)
pineal = data[0].copy()
frontal = data[1].copy()
...
if p_pineal > 0.05:
    print("Warning: Pineal signal is not stationary — differencing applied")
    pineal = np.diff(pineal)
if p_frontal > 0.05:
    print("Warning: Frontal signal is not stationary — differencing applied")
...
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('plots/s01_dmt_20_granger_causality_pvalues.png', dpi=600)
print("Plot 20 done: s01_dmt_20_granger_causality_pvalues.png")

# 21. Correlation-based connectivity graph (no self-loops)
...
plt.savefig('plots/s01_dmt_21_correlation_based_connectivity_graph.png', dpi=600)
print("Plot 21 done: s01_dmt_21_correlation_based_connectivity_graph.png")
...
plt.grid(True)
plt.savefig('plots/s01_dmt_22_permutation_entropy_vs_time.png', dpi=600)
print("Plot 22 done: s01_dmt_22_permutation_entropy_vs_time.png")
...
plt.grid(True)
plt.savefig('plots/s01_dmt_23_hilbert_instant_freq_43hz.png', dpi=600)
print("Plot 23 done: s01_dmt_23_hilbert_instant_freq_43hz.png")
...
plt.grid(True)
plt.savefig('plots/s01_dmt_24_theta_power_vs_time.png', dpi=600)
print("Plot 24 done: s01_dmt_24_theta_power_vs_time.png")
...
plt.grid(True)
plt.savefig('plots/s01_dmt_25_43hz_power_vs_time.png', dpi=600)
print("Plot 25 done: s01_dmt_25_43hz_power_vs_time.png")
...
plt.tight_layout()
plt.savefig('plots/s01_dmt_26_cross_frequency_correlation_heatmap.png', dpi=600)
print("Plot 26 done: s01_dmt_26_cross_frequency_correlation_heatmap.png")
...
plt.grid(True)
fig.tight_layout()
plt.savefig('plots/s01_dmt_27_delta_alpha_mse_overlay.png', dpi=600)
print("Plot 27 done: s01_dmt_27_delta_alpha_mse_overlay.png")
...
plt.grid(True)
plt.savefig('plots/s01_dmt_28_gamma_bursts_density_on_theta.png', dpi=600)
print("Plot 28 done: s01_dmt_28_gamma_bursts_density_on_theta.png")

# Cohen's d (defined here, BEFORE Plot 29)
def cohens_d(a, b):
...
# 29. Δα and Gamma Power Pre vs Post t=41 s (Main Effect Summary)
...
plt.grid(True, alpha=0.3)
fig.tight_layout()
plt.savefig('plots/s01_dmt_29_delta_alpha_gamma_pre_post_summary.png', dpi=600)
print("Plot 29 done: s01_dmt_29_delta_alpha_gamma_pre_post_summary.png")

# 30. Research-Grade Summary Visual
fig, ax1 = plt.subplots(figsize=(14,7))
...
plt.tight_layout()
plt.savefig('plots/s01_dmt_30_summary_visual.png', dpi=600)
print("Plot 30 done: s01_dmt_30_summary_visual.png")

# =============================================================================
# RESEARCH-GRADE SUMMARY & CSV EXPORT + PROFESSIONAL PDF REPORT WITH DYNAMIC CAPTIONS
# =============================================================================
...
summary = f"""ALADIN S01-DMT Validation Summary
====================================
PAC (Tort MI): {mi:.4f}
...
with open('plots/s01_dmt_summary_stats.txt', 'w') as f:
    f.write(summary)

print("Summary stats saved: s01_dmt_summary_stats.txt")
...
timecourses.to_csv('plots/s01_dmt_timecourses.csv', index=False)
print("Timecourses CSV saved: s01_dmt_timecourses.csv")

# ====================== PROFESSIONAL PDF REPORT WITH DYNAMIC CAPTIONS ======================
print("\n=== GENERATING PROFESSIONAL PDF REPORT WITH DYNAMIC CAPTIONS ===")

plot_dir = 'plots'
plot_files = sorted([f for f in os.listdir(plot_dir) if f.startswith('s01_dmt_') and f.endswith('.png')])

# Dynamic captions with precise t_peak in ALL
plot_captions = {
    's01_dmt_01_fluctuation_functions.png': f'Full-signal MF-DFA fluctuation functions across q orders (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_02_hq_curve.png': f'MF-DFA generalized Hurst exponent h(q) curve showing multifractality (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_03_singularity_spectrum.png': f'MF-DFA singularity spectrum f(α) – width indicates multifractal strength (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_04_delta_alpha_timecourse.png': f'Time-resolved Δα collapse with objective peak at {t_peak_precise:.6f} s',
    's01_dmt_05_delta_alpha_gamma_43vs40.png': f'Δα collapse aligned with 40–50 Hz gamma power (peak at {t_peak_precise:.6f} s)',
    's01_dmt_06_delta_alpha_aaft_surrogate.png': f'AAFT surrogate test with objective peak at {t_peak_precise:.6f} s',
    's01_dmt_10_surrogate_zscore_histogram.png': f'Surrogate distribution of Δα at peak time (real value marked, t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_07_delta_alpha_pre_post_permutation.png': f'Δα pre vs post t≈{t_peak_precise:.6f} s with permutation test',
    's01_dmt_08_gamma43_pre_post_permutation.png': f'Gamma 40–50 Hz power pre vs post t≈{t_peak_precise:.6f} s with permutation test',
    's01_dmt_09_psd_pre_post.png': f'Power spectral density pre vs post t≈{t_peak_precise:.6f} s switch',
    's01_dmt_11_superradiance_mt_disorder.png': f'Microtubule Superradiance Coherence Driven by 43 Hz Gamma (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_12_alpha_power_vs_time.png': f'Alpha (8–12 Hz) power timecourse (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_13_beta_power_vs_time.png': f'Beta (12–30 Hz) power timecourse (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_14_buzsaki_theta_43hz_pac.png': f'Buzsáki-style theta–43 Hz phase-amplitude coupling (Tort MI) (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_15_buzsaki_gamma_bursts_on_theta.png': f'Gamma bursts locked to theta cycles (Buzsáki canonical) (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_16_multitaper_lfp_power_spectrum.png': f'Multitaper LFP power spectrum showing 43 Hz peak (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_17_lfp_multiscale_entropy.png': f'True multiscale sample entropy of LFP signal (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_18_izhi_network_rate.png': f'Izhikevich spiking network driven by observed 43 Hz gamma (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_19_izhi_mfdfa_output.png': f'MF-DFA applied to Izhikevich network output (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_20_granger_causality_pvalues.png': f'Granger causality pineal → frontal with FDR correction (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_21_correlation_based_connectivity_graph.png': f'Correlation-based EEG connectivity graph (no self-loops) (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_22_permutation_entropy_vs_time.png': f'Permutation entropy timecourse (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_23_hilbert_instant_freq_43hz.png': f'Instantaneous frequency of 43 Hz gamma component (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_24_theta_power_vs_time.png': f'Theta (4–8 Hz) power timecourse (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_25_43hz_power_vs_time.png': f'43 Hz gamma power timecourse (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_26_cross_frequency_correlation_heatmap.png': f'Cross-frequency power correlation heatmap (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_27_delta_alpha_mse_overlay.png': f'Overlay of Δα and multiscale entropy timecourses (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_28_gamma_bursts_density_on_theta.png': f'2D density of gamma amplitude on theta phase (t_peak ≈ {t_peak_precise:.6f} s)',
    's01_dmt_29_delta_alpha_gamma_pre_post_summary.png': f'Δα and gamma power pre vs post t≈{t_peak_precise:.6f} s with Cohen’s d and p-values',
    's01_dmt_30_summary_visual.png': f'Research-grade summary: Δα collapse & gamma power with surrogate band and objective peak at {t_peak_precise:.6f} s'
}

with open(os.path.join(plot_dir, 's01_dmt_summary_stats.txt'), 'r') as f:
    summary_text = f.read()

pdf_path = os.path.join(plot_dir, 'S01_DMT_Validation_Report.pdf')
with PdfPages(pdf_path) as pdf:
    # Title Page
    plt.figure(figsize=(8.5, 11))
    plt.text(0.5, 0.7, "ALADIN EEG Validation Suite – S01-DMT", fontsize=24, ha='center')
    ...
    # All Plots with Dynamic Captions
...
print(f"PDF report generated: {pdf_path} (with t_peak in ALL captions)")

print("\nAll 30 beautiful validations complete!")
print("All plots saved with s01_dmt_ prefix.")
