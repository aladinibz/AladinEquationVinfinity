...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.6 – 50k steps – KINK SEED + SAUSAGE TEST        ║
# ║  (128³ grid, strong kink seed to break symmetry, ultra-weak diffusion)     ║
# ╚════════════════════════════════════════════════════════════════════════════╝

GRID_SIZE = 128
MAX_STEPS = 50000          # quick 50k test to see if kink helps sausage
dt_max    = 1.2e-4
CFL       = 0.75

print("🚀 ALADIN v1.6 – 50k steps – KINK SEED + SAUSAGE TEST")
print("Strong m=1 kink seed added to break symmetry – let's see if sausage wakes up!")
...
k_sausage = cp.float64(0.7)  # tuned for fastest sausage growth
...
# Initial conditions – STRONG SAUSAGE + VERY STRONG KINK SEED
...
rho += 1.0 * rho0 * cp.cos(k_sausage * Z)  # strong sausage beads
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.6 – 50k steps – SAUSAGE + KINK TEST")
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.6 – 50,000 steps – FINAL ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    plt.grid(alpha=0.3)
    plt.legend()
    plt.savefig('plots/plot35_filtered_43hz_signals_50k_test.png', dpi=200)
    plt.close()
    print("Saved: plots/plot35_filtered_43hz_signals_50k_test.png")
...
plt.ylabel('Power')
plt.title('Fermion Mass z-Spatial Power Spectrum – 50k steps test')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_50k_test.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_50k_test.png")
...
plt.ylabel('Power')
plt.title('Azimuthal Power Spectrum – Kink Detection (50k test)')
plt.axvline(1, color='red', ls='--', label='m=1 kink')
plt.legend()
plt.grid(alpha=0.3)
plt.savefig('plots/plot45_azimuthal_power_kink_50k_test.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot45_azimuthal_power_kink_50k_test.png")

print("\nALADIN v1.6 – 50,000 steps test finished!")
print("Strong sausage + kink seeds added – check sausage band and m=1 power!")
