"""
S11-DMT Breakthrough — Complete Analysis with 12 Proof Plots
High-dose DMT session
Full MF-DFA + short-window Δα timecourse + joint gamma (43 vs 40 Hz) + AAFT surrogates on Δα & gamma bands + pre/post box plots with CI + PSD pre/post + Z-score histogram
ALADIN ∞ ℂ(t) — The Final Law
January 16, 2026 — One-file version, 12 PNGs generated

Downloads S11-DMT.bdf directly from repo
"""
...
plt.ylabel('F(q,s)')
plt.grid(True)
plt.savefig('plots/s11_dmt_fluctuation_functions.png', dpi=300)
plt.close()
...
plt.ylabel('h(q)')
plt.grid()
plt.savefig('plots/s11_dmt_hq_curve.png', dpi=300)
plt.close()
...
plt.ylabel('f(α)')
plt.grid()
plt.savefig('plots/s11_dmt_singularity_spectrum.png', dpi=300)
plt.close()
...
plt.axvline(41, color='gold', linestyle='--', linewidth=3, label='t=41 s switch')
plt.title('Δα Collapse (S11-DMT)')
plt.xlabel('Time (s)')
plt.ylabel('Δα')
plt.grid(True)
plt.legend()
plt.savefig('plots/s11_dmt_delta_alpha_timecourse.png', dpi=300)
plt.close()
...
plt.title('Δα vs Gamma (S11-DMT)')
plt.xlabel('Time (s)')
plt.tight_layout()
plt.savefig('plots/s11_dmt_delta_alpha_gamma_43vs40.png', dpi=300)
plt.close()
...
plt.axvline(41, c='red', ls='--', lw=3, label='t=41 s')
plt.title('AAFT Surrogate Test (S11-DMT)')
plt.xlabel('Time (s)')
plt.ylabel('Δα')
plt.legend(fontsize=10)
plt.grid(True, alpha=0.3)
plt.savefig('plots/s11_dmt_delta_alpha_aaft_surrogate.png', dpi=300)
plt.close()
...
plt.ylabel('Δα')
plt.grid(True, alpha=0.3)
plt.savefig('plots/s11_dmt_delta_alpha_pre_post_ci.png', dpi=300)
plt.close()
...
plt.grid(True, alpha=0.3)
plt.savefig('plots/s11_dmt_gamma43_pre_post_ci.png', dpi=300)
plt.close()

# PSD Pre vs Post
pre_signal = signal[:int(41*60*128)]
post_signal = signal[int(41*60*128):]
...
plt.title('PSD Pre vs Post t=41 s Switch (S11-DMT)')
plt.axvspan(40, 50, color='gold', alpha=0.15, label='40–50 Hz band')
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('plots/s11_dmt_psd_pre_post.png', dpi=300)
plt.close()
...
plt.title('Z-scores at t≈41 s across surrogates (S11-DMT)')
plt.xlabel('Z-score')
plt.ylabel('Count')
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('plots/s11_dmt_zscore_histogram.png', dpi=300)
plt.close()

print("All 12 plots generated in 'plots/' folder!")
