...
print("🌌 ALADIN Plasma Cosmology v38.9 — FULL FUSED RAWKERNEL CT YEE (Shapes Fixed)")
...
kernel_code = r'''
extern "C" __global__
void fused_ct_yee_kernel(float* vx, float* vy, float* vz,
...
kernel = cp.RawKernel(kernel_code, 'fused_ct_yee_kernel')
...
print("Starting v38.9 with FULL FUSED RAWKERNEL...")
...
for step in range(steps):
    ...
    if step % 50 == 0:
        ...
        # FIXED: Cell-centered B for magnitude (solves shape error)
        Bx_c = 0.5 * (Bx[1:,:,:] + Bx[:-1,:,:])
        By_c = 0.5 * (By[:,1:,:] + By[:,:-1,:])
        Bz_c = 0.5 * (Bz[:,:,1:] + Bz[:,:,:-1])
        Bmag = cp.sqrt(Bx_c**2 + By_c**2 + Bz_c**2)
        Bmax = float(cp.max(Bmag))
        
        print(f"Step {step:4d} | Bmax = {Bmax:.2f} μG | vmax = {vmax:.1f} km/s | divB_max = {div_max:.2e}")

print("\n✅ v38.9 Finished! Fused RawKernel + Proper staggered handling.")
