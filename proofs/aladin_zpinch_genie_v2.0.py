...
# ==================== PARAMETERS ====================
GRID_SIZE = 128
MAX_STEPS = 120000       # <- stop exactly at 120k steps
dt_max    = 1.2e-4
CFL       = 0.75
...
# Apply radial Bessel J0 profile along z (sausage)
j0_3d = j0_kr[:, cp.newaxis, cp.newaxis]
rho += 1.0 * rho0 * j0_3d * cp.cos(k_sausage * Z)

# Single kink (m=1) azimuthal perturbation
...
while step < MAX_STEPS:
    ...
    gamma_rel = cp.clip(1.0 / cp.sqrt(1.0 - v2/c**2 + 1e-18), 1.0, 15.0)
    ...
    local_damping = kg_damping*(4.0 if current_amp>1 else 1.0)
    ...
    pineal_res = 0.18*cp.sin(2*np.pi*43*t + 0.15)
    ...
    E_total = float(cp.mean(0.5*rho*(v_r**2+v_phi**2+v_z**2)) + cp.mean(0.5*(B_r**2+B_phi**2+B_z**2)/mu0))
...
peak_power_genie = power_genie[np.argmin(np.abs(freq-43.0))]

verdict = "❌ no clear 43Hz"
if peak_power_genie>1e4: verdict="✅ STRONG 43Hz LOCK (filtered)"
print(f"43Hz verdict: {verdict}")
