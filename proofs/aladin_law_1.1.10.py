"""
ALADIN v1.1.10 — 128³ LOCKED - Final Stable Version
"""
...
print("🚀 ALADIN v1.1.10 — 128³ LOCKED - Stable Version\n")
...
GAMMA_BULK = 5.0   # Change this to test different relativistic regimes
    ...
    for rho0 in rho0_list:
        ...
        for step in tqdm(range(n_steps), leave=False, desc=f"J0={J0:.1e}"):
            ...
            if step % save_every == 0:
                ...
                m0 = float(np.asarray(power_m[0]).item())
                m1 = float(np.asarray(power_m[1]).item()) if len(power_m) > 1 else 0.0
