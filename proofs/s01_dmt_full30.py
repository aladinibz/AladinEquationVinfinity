...
# True sub-millisecond t_peak using upsampled gamma envelope (0.488 ms resolution)
target_fs = 2048
...
# AAFT surrogate on Δα
n_sur = CONFIG['n_sur']
surr_d = np.zeros((n_sur, len(time_sec)))

print("Computing AAFT surrogates...")
for i in tqdm(range(n_sur), desc="AAFT surrogates"):
    try:
...
plt.plot(t_sup * 1e9, intensity_disorder / np.max(intensity_disorder), label='With disorder (σ=1 GHz)', color=COLORS['gamma'], lw=2.5, ls='--')
plt.xlabel('Time (ns)')
...
plot_captions = {
    ...
    's01_dmt_11_superradiance_mt_disorder.png': f'Theoretical superradiance in microtubule bundle with disorder (t_peak ≈ {t_peak_precise:.6f} s)',
...
with PdfPages(pdf_path) as pdf:
    ...
    # All Plots with Dynamic Captions (t_peak in ALL)
