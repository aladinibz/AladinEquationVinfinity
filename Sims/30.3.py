...
for step in range(steps):
    ...
    # FIXED SHAPE: update internal cells only
    ...
    # STAGE 2 (identical pattern with same fixed slices)
    # x-sweep stage 2
    rho_L, rho_R = muscl_reconstruct(rho, 0)
    mx_L, mx_R = muscl_reconstruct(mx, 0)
    my_L, my_R = muscl_reconstruct(my, 0)
    ...
    # FIXED SHAPE CT update
    Bx[1:-1,:,:] += (dt / dx) * ((Ez[:,1:,:] - Ez[:,:-1,:])[:,:-1,:] - (Ey[:,:,1:] - Ey[:,:,:-1])[:,:,:-1])
    By[:,1:-1,:] += (dt / dx) * ((Ex[:,:,1:] - Ex[:,:,:-1])[:,:,:-1] - (Ez[1:,:,:] - Ez[:-1,:,:])[1:-1,:,:-1])
    Bz[:,:,1:-1] += (dt / dx) * ((Ey[1:,:,:] - Ey[:-1,:,:])[1:-1,:,:] - (Ex[:,1:,:] - Ex[:,:-1,:])[:,:-1,1:-1])
...
print("\n✅ v33.3 complete! Shape mismatch FIXED in flux updates + CT block.")
