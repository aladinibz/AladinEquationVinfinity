...
plt.rcParams.update({
    ...
    'lines.linewidth': 2.8, 'axes.grid': True, 'grid.alpha': 0.35,
    'grid.linestyle': '--', 'axes.facecolor': '#0f0f0f',
    'figure.facecolor': '#050505', 'text.color': 'white',
    'axes.edgecolor': '#555555', 'xtick.color': 'white',
    'ytick.color': 'white', 'savefig.dpi': 600, 'savefig.bbox': 'tight'
})
...
print("=== ALADIN S01-DMT VALIDATION SUITE – 32 PLOTS (COMPLETE FINAL) ===")
...
def cohens_d(a, b):
    ...
    return (np.nanmean(a) - np.nanmean(b)) / s if s > 1e-12 else np.nan
...
gamma_sig = bandpass(signal, 40, 50, fs)
...
n_win = max(0, (len(signal) - win_samples) // step_samples)
for i in tqdm(range(n_win), desc="Δα windows"):
    ...
    lag_win, dfa_win = MFDFA(win, lag=lag, q=q, order=1)
    w_hq = []
    ...
    deltas.append(np.nanmax(w_alpha) - np.nanmin(w_alpha))
deltas = np.array(deltas)
time_sec_d = np.array(time_sec_d)

# CRITICAL EARLY idx_peak
...
pre_g = gamma_power[:len(pre_d)]
post_g = gamma_power[len(pre_d):len(pre_d) + len(post_d)]

obs_d, p_perm_d = perm_test(pre_d, post_d)
obs_g, p_perm_g = perm_test(pre_g, post_g)

# ====================== SURROGATES ======================
print("Computing shuffle surrogates...")
...
def compute_one_surrogate(i):
    ...
    for j in range(n_win):
        ...
        if end > len(surr): break
        win = surr[start:end]
        lag_win, dfa_win = MFDFA(win, lag=lag, q=q, order=1)
        w_hq = []
        for k in range(len(q)):
            valid = np.where((dfa_win[k] > 0) & np.isfinite(dfa_win[k]))[0]
            if len(valid) > 8:
                fit = np.polyfit(np.log(lag_win[valid]), np.log(dfa_win[k, valid]), 1)
                w_hq.append(fit[0])
            else:
                w_hq.append(np.nan)
        w_tau = q * np.array(w_hq) - 1
        w_alpha = np.gradient(w_tau, q)
        surr_deltas.append(np.nanmax(w_alpha) - np.nanmin(w_alpha))
    return np.array(surr_deltas)
...
# ====================== MULTITAPER ======================
psds_mt, freqs_mt = psd_array_multitaper(
    signal, sfreq=fs, fmin=1, fmax=60,
...
plt.axvline(43, color='#ffd700', ls='--', lw=3, label='43 Hz Core Frequency')
...
ax2 = ax1.twinx()
ax2.plot(time_sec, gamma_power, color=COLORS['gamma'], lw=7, label='43 Hz Gamma – CORE FOUNDATION')
ax2.axvline(t_peak, color='#ff0000', ls='--', lw=4)
ax2.text(t_peak + 0.5, gamma_power.max()*0.9, 'GAMMA CORE FOUNDATION', color=COLORS['gamma'], fontsize=16, fontweight='bold')
...
plt.title('04. 43 Hz instantaneous frequency + power timecourse')
plt.xlabel('Time (s)')
...
for i in range(len(q)):
    plt.loglog(lag, dfa[i], lw=2, label=f'q={q[i]:.1f}')
...
for i in range(len(q)):
    ...
    if len(valid) > 8:
        fit = np.polyfit(np.log(lag[valid]), np.log(dfa[i, valid]), 1)
        hq.append(fit[0])
    else:
        hq.append(np.nan)
hq = np.array(hq)
...
plt.figure(figsize=(11,6.5))
plt.plot(alpha_spec, f_alpha, 'ro-', lw=4, color=COLORS['delta_alpha'])
...
plt.title('08. Sliding-window Δα timecourse (with t_peak line)')
plt.xlabel('Time (s)')
plt.ylabel('Δα')
plt.legend()
plt.grid(alpha=0.4)
...
plt.fill_between(time_sec_d, mean_s - 2*std_s, mean_s + 2*std_s, color='gray', alpha=0.3, label='Surrogate ±2σ')
plt.axvline(t_peak, color='#ff0000', ls='--', lw=4, label=f't_peak = {t_peak:.3f} s')
plt.title('09. Shuffle surrogate test + Z-score')
plt.xlabel('Time (s)')
plt.ylabel('Δα')
plt.legend()
plt.grid(alpha=0.4)
...
plt.title('10. Δα vs surrogate band overlay')
plt.xlabel('Z-score')
plt.ylabel('Count')
plt.grid(alpha=0.4)
...
plt.title('11. Pre/post permutation tests (Δα + gamma)')
plt.ylabel('Δα')
plt.grid(alpha=0.4)
...
plt.fill_between(t_zoom, mean_s_zoom - 2*std_s_zoom, mean_s_zoom + 2*std_s_zoom, color='gray', alpha=0.3, label='Surrogate ±2σ')
ax2 = plt.twinx()
ax2.plot(t_zoom, gamma_zoom, color=COLORS['gamma'], lw=5, label='43 Hz Gamma')
plt.axvline(t_peak, color='#ff0000', ls='--', lw=4, label=f't_peak = {t_peak:.3f} s')
...
plt.title('15. Buzsáki Theta–43 Hz PAC (Tort MI)')
...
plt.axvline(43, linestyle='--', color='#ff0000', lw=3)
plt.xlabel("Frequency (Hz)")
...
# Plot 19
gamma_band = bandpass(signal, 41, 45, fs)
...
plt.title('31. Key composite 2 (your current style)')
plt.xlabel('Time (s)')
plt.legend(loc='upper left')
ax2.legend(loc='upper right')
plt.grid(alpha=0.4)
...
np.random.seed(CONFIG['seed'])
CONFIG['n_perm'] = 5000
CONFIG['analysis_timestamp'] = str(datetime.now())
...
delta_pct = (
    np.nan if abs(np.nanmean(pre_d)) < 1e-12
    else 100 * (np.nanmean(pre_d) - np.nanmean(post_d)) / np.nanmean(pre_d)
)
...
sur_p = (
    (np.sum(np.abs(surr_d[:, idx_peak]) >= np.abs(real_val)) + 1) / (surr_d.shape[0] + 1)
    if idx_peak < surr_d.shape[1] and not np.isnan(real_val) else np.nan
)
...
if np.isnan(delta_pct):
    collapse_direction = "Unknown (NaN delta_pct)"

delta_auc_abs = np.trapz(np.abs(deltas), time_sec_d)
delta_auc_signed = np.trapz(deltas, time_sec_d)
...
valid_mask = ~np.isnan(deltas) & ~np.isnan(gamma_power[:len(deltas)])
corr_delta_gamma = np.corrcoef(deltas[valid_mask], gamma_power[:len(deltas)][valid_mask])[0,1] if np.sum(valid_mask) > 1 else np.nan
...
if p_perm_d < 0.001:
    interpretation = "Highly significant Δα reduction detected (*** p < 0.001)"
elif p_perm_d < 0.01:
    interpretation = "Significant Δα reduction detected (** p < 0.01)"
elif p_perm_d < 0.05:
    interpretation = "Significant Δα reduction detected (* p < 0.05)"
else:
    interpretation = "No significant Δα reduction (ns)"
...
PRIMARY ENDPOINT
{'-'*60}
Δα pre/post change anchored to gamma peak (t_peak)

CORE ALIGNMENT
{'-'*60}
Gamma peak (t_peak)          : {t_peak:.6f} s
PAC (Tort MI)                : {mi:.4f}
Δα at peak                   : {real_val:.4f}
Z-score at t_peak            : {z:.2f} σ
Surrogate empirical p        : {sur_p:.6f}
Time of max collapse         : {time_of_max_collapse:.3f} s
Time-lock consistency        : {time_lock_diff:+.3f} s
...
Δα AUC (absolute)            : {delta_auc_abs:.4f}
Δα AUC (signed)              : {delta_auc_signed:.4f}
Collapse duration            : {collapse_duration:.2f} s  (baseline-anchored)
Fixed-window mean (±0.5 s)   : {fixed_window_mean:.4f}
Baseline stability (pre std) : {baseline_std:.4f}
SNR (effect / noise)         : {snr_delta:.3f}
Δα–Gamma coupling            : {corr_delta_gamma:.3f}

STATISTICAL VALIDATION (α = 0.05, two-tailed)
{'-'*60}
...
Window length                : {CONFIG.get('window_sec', 5)} s
Step                         : {CONFIG.get('step_sec', 4)} s
Surrogates (n)               : {CONFIG.get('n_sur', 200)}
Permutation iterations       : {CONFIG.get('n_perm', 5000)}
Valid Δα windows             : {n_valid_windows}
Random seed                  : {CONFIG.get('seed', 42)}
Analysis timestamp           : {CONFIG.get('analysis_timestamp', 'N/A')}

Author                       : {CONFIG.get('author', 'Unknown')}
Subject / Condition          : {subject_id} / {condition}
Total plots                  : 32
...
with open(f'plots/{prefix}_summary_stats.txt', 'w') as f:
    f.write(summary)
print(f"Summary stats saved: plots/{prefix}_summary_stats.txt")
...
subject_metrics = pd.DataFrame([{
    ...
    "delta_auc_signed": delta_auc_signed,
...
with PdfPages(pdf_path) as pdf:
    ...
    plt.text(0.5, 0.7, f"ALADIN {subject_id} {condition} Validation Suite – 32 Plots",
             fontsize=22, ha='center')
    plt.text(0.5, 0.62, "Clean 8-Section Narrative Structure",
             fontsize=14, ha='center')
    plt.text(0.5, 0.55, f"Author: {CONFIG.get('author', 'Unknown')}",
...
print("\n✅ COMPLETE FULL SCRIPT WITH ALL 32 PLOTS. Run it bro!")
print("Love you brother 💪 Now start the subjects. This is the final version.")
