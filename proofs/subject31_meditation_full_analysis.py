"""
Subject 31 Meditation — Complete Analysis (All 8 Proof Plots)
3-hour natural sustained meditation session
Full MF-DFA + timecourse + joint gamma (43 vs 40 Hz) + AAFT surrogate + pre/post box plots with CI
ALADIN ∞ ℂ(t) — The Final Law
January 16, 2026 — One-file version, 8 PNGs generated

Run this file once to generate all 8 plots in /plots/
"""
...
n43 = np.zeros(len(time_min)) if np.isnan(np.nanmax(gamma43)) or np.nanmin(gamma43) == np.nanmax(gamma43) else (gamma43 - np.nanmin(gamma43)) / (np.nanmax(gamma43) - np.nanmin(gamma43))
n40 = np.zeros(len(time_min)) if np.isnan(np.nanmax(gamma40)) or np.nanmin(gamma40) == np.nanmax(gamma40) else (gamma40 - np.nanmin(gamma40)) / (np.nanmax(gamma40) - np.nanmin(gamma40))
...
ax2 = plt.gca().twinx()
ax2.plot(time_min, n43, 'gold-', lw=4, label='40–50 Hz')
ax2.plot(time_min, n40, 'cyan--', lw=4, label='38–42 Hz')
...
pre_g43 = gamma_powers_43[:t41][~np.isnan(gamma_powers_43[:t41])]
post_g43 = gamma_powers_43[t41:][~np.isnan(gamma_powers_43[t41:])]
