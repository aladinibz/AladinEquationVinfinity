...
# AAFT surrogate on Δα — OPTIMIZED WITH PARALLEL JOBLIB + TQDM
n_sur = CONFIG['n_sur']
...
print("Computing AAFT surrogates (parallelized with progress bar)...")
surr_list = Parallel(n_jobs=-1)(delayed(compute_one_surrogate)(i, signal, time_sec, step_samples, win_samples, q) for i in tqdm(range(n_sur), desc="AAFT surrogates"))
...
# 16. Welch LFP Power Spectrum (light version to save RAM)
...
plot_captions = {
    ...
    's01_dmt_16_multitaper_lfp_power_spectrum.png': f'Welch LFP power spectrum showing 43 Hz peak (t_peak ≈ {t_peak_precise:.6f} s)',
