...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.3 – 5000 steps version                          ║
# ║  (128³ grid, tuned couplings, visible Majorana, 43 Hz + fermion spectra)   ║
# ╚════════════════════════════════════════════════════════════════════════════╝

TEST_MODE = True

GRID_SIZE = 128
MAX_STEPS = 5000           # ← as requested: 5000 steps for more evolution
dt_max    = 5e-6           # increased to allow meaningful physical time
CFL       = 0.35           # slightly looser for larger dt

print("Running 5000 steps on 128³ grid – should take ~5–7 min on A100")
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.3 – 5000 steps evolution")
...
while t < t_max and step < MAX_STEPS:
    ...
    dt = cp.maximum(dt, 1e-7)                  # ← prevents dt from becoming too small
    dt_prev = dt
    t += dt
    ...
    if step % 500 == 0:
        ...
        print(f"Step {step:5d} | t = {float(t):.6f} | dt = {float(dt):.2e} | "
              f"genie_mean = {history['genie_amp'][-1]:.2e} | "
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.3 – 5000 steps – 43 Hz + FERMION SPATIAL SPECTRA ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    plt.savefig('plots/plot43hz_detection_v13_5000steps.png', dpi=220, facecolor='black')
    plt.close()
    print("Saved: plots/plot43hz_detection_v13_5000steps.png")
...
plt.ylabel('Power')
plt.title('Fermion Mass Spatial Power Spectrum – 5000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_5000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_5000.png")
...
plt.xlabel('Wavenumber k (1/a)')
plt.ylabel('Power')
plt.title('Fermion Density Spatial Power Spectrum – 5000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot44_fermion_density_spatial_spectrum_5000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot44_fermion_density_spatial_spectrum_5000.png")

print("\nALADIN v1.3 – 5000 steps finished!")
print("Check the plots folder for updated spectra and 43 Hz detection.")
print("Especially look at simulated time span and whether spectra show structure.")
