...
print("🌌 Plasma Cosmology v6.6 — 128³ A100 GPU + Radiative Cooling + Cosmic Ray Diffusion")
...
# Cosmic ray diffusion coefficient (galactic scale ~10^28 cm²/s → code units)
kappa_cr = cp.float32(1.0e-3)   # tune this: higher = stronger diffusion
...
def run_simulation(use_dm, use_cr=True):
    ...
    for step in range(steps):
        ...
        # === COSMIC RAY TRANSPORT (advection + diffusion) ===
        if use_cr:
        ...
        # Radiative cooling (unchanged)
        T = p_thermal * (1.4 * mH) / (rho * kB)
        T = cp.maximum(T, 100.0)
...
plt.title('Rotation Curves — Side-by-Side (CR diffusion + cooling)')
plt.legend()
plt.grid(True)
...
print("✅ v6.6 complete! Share the new rotation curve shapes, Bmax/vmax, energy drifts, and how CR diffusion affects the results.")
