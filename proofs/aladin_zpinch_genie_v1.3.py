...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.3 – 50k steps + explicit 43 Hz pump             ║
# ║  (128³ grid, strong dt ramp, Majorana domination, forced resonance)        ║
# ╚════════════════════════════════════════════════════════════════════════════╝

TEST_MODE = True

GRID_SIZE = 128
MAX_STEPS = 50000          # 50k steps – full send
dt_max    = 4e-5           # 40 µs ceiling – bigger for more time
CFL       = 0.55           # push harder

print("🚀 ALADIN v1.3 – 50k steps + EXPLICIT 43 Hz PUMP")
print(f"dt_max = {dt_max:.1e}, CFL = {CFL}, expected runtime ~30–60 min on A100")
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.3 – 50k steps + explicit 43 Hz pump")
...
while t < t_max and step < MAX_STEPS:
    ...
    # MAXIMUM early ramp-up: first 15k steps push dt HARD
    if step < 15000 and t < 0.1:
        dt_new = cp.maximum(dt_new, 3e-4)          # min 300 µs early
        if t < 0.02:
            dt_new = cp.minimum(dt_new * 1.9, 6e-4)  # extreme exponential boost

    dt = 0.7 * dt_prev + 0.3 * dt_new
    dt = cp.minimum(dt, dt_max)
    dt = cp.maximum(dt, 1e-7)
    ...
    # Genie scalar + EXPLICIT 43 Hz PUMP
    source_genie = g_genie * J_z + majorana_backreaction
    source_genie += 5e-4 * cp.sin(2 * cp.pi * 43 * t)  # ← explicit 43 Hz pump – amplitude 5e-4
    source_genie = cp.clip(source_genie, -1500, 1500)

    lap_now = cyl_laplacian(genie_phi)
    accel = lap_now - m_phi**2 * genie_phi - 0.005 * genie_phi**3 + source_genie
    accel = cp.clip(accel, -6e4, 6e4)
    ...
    genie_phi_new -= 0.012 * cyl_laplacian(genie_phi_new) * dt
    genie_phi_new = cp.clip(genie_phi_new, -1.5, 1.5)
    ...
    C_field = cp.clip(C_field, 0.0, 15.0)
    ...
    if step % 2500 == 0:
        ...
        print(f"Step {step:5d} | t = {float(t):.6f} | dt = {float(dt):.2e} | "
    ...
    if step % 10000 == 0 and step > 0:
        checkpoint_file = f"checkpoints/checkpoint_step{step:05d}.npz"
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.3 – 50,000 steps – FINAL ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    power_genie = np.abs(rfft(sig_genie_u))**2
    power_C = np.abs(rfft(sig_C_u))**2

    idx_43 = np.argmin(np.abs(freq - 43.0))
    window_start = max(0, idx_43 - 25)
    window_end = min(len(freq), idx_43 + 26)

    peak_freq_g = 0.0
    peak_power_g = 0.0

    if window_end > window_start and len(freq) > 0:
        window_slice = slice(window_start, window_end)
        local_power = power_genie[window_slice]
        if len(local_power) > 0:
            peak_offset = np.argmax(local_power)
            peak_idx_g = window_start + peak_offset
            peak_freq_g = freq[peak_idx_g]
            peak_power_g = local_power[peak_offset]

    print(f"Simulated time span : {t_np[-1]:.6f}")
    print(f"Frequency resolution: {freq[1] if len(freq) > 1 else 0:.4f} Hz")
    print(f"Peak frequency (Genie center): {peak_freq_g:.4f} Hz")
    print(f"Power at peak              : {peak_power_g:.2e}")
    print(f"Power at exact 43 Hz       : {power_genie[idx_43] if len(power_genie) > idx_43 else 0:.2e}")

    verdict = "❌ no clear 43 Hz"
    if abs(peak_freq_g - 43.0) < 0.7:
        verdict = "✅ STRONG 43 Hz LOCK"
    elif abs(peak_freq_g - 43.0) < 3.0:
        verdict = "⚠️ weak/near 43 Hz signal"
    ...
    plt.semilogy(freq, power_genie, 'cyan', lw=1.8, label='Genie ϕ (center)')
    plt.semilogy(freq, power_C,     'magenta', lw=1.6, alpha=0.85, label='Mean C_field')
    plt.axvline(43.0, color='red', ls='--', lw=2.8, label='Target 43 Hz')
    if peak_freq_g > 0:
        plt.axvline(peak_freq_g, color='lime', ls=':', lw=2.2, label=f'Peak @ {peak_freq_g:.3f} Hz')
    plt.xlim(0, 120)
    plt.ylim(1e-10, None)
    plt.xlabel('Frequency [Hz]', color='white')
    plt.ylabel('Power', color='white')
    plt.title(f'43 Hz Resonance Detection – 50000 steps – {verdict}', color='white')
    plt.legend()
    plt.grid(alpha=0.25, which='both', color='gray')
    plt.savefig('plots/plot43hz_detection_v13_50000steps.png', dpi=220, facecolor='black')
    plt.close()
    print("Saved: plots/plot43hz_detection_v13_50000steps.png")
else:
    print("Not enough time points for 43 Hz FFT (need >200 steps)")
...
plt.ylabel('Power')
plt.title('Fermion Mass Spatial Power Spectrum – 50000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_50000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_50000.png")
...
plt.xlabel('Wavenumber k (1/a)')
plt.ylabel('Power')
plt.title('Fermion Density Spatial Power Spectrum – 50000 steps')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot44_fermion_density_spatial_spectrum_50000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot44_fermion_density_spatial_spectrum_50000.png")

# Time warning
if t_np[-1] < 0.02:
    print("WARNING: Time span still < 0.02 s – dt ramp not aggressive enough. Try dt_max = 5e-5 or GRID_SIZE = 64 next.")
else:
    print("SUCCESS: Time span > 0.02 s – dynamics should be visible now!")

print("\nALADIN v1.3 – 50,000 steps finished!")
print("Drop the final time span, 43 Hz verdict, dominant k, Majorana %, genie amp, and any plot observations.")
