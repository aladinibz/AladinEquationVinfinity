...
// === Refined Adaptive Geodesic Strength ===
__device__ float compute_geodesic_strength(float entropy_force, float trace_P, float det_P) {
    float base = 0.92f;
    float entropy_boost = fminf(0.06f, 0.03f * fabsf(entropy_force));

    // Condition-aware boost (smaller det relative to trace^3 = worse conditioned)
    float cond = (trace_P * trace_P * trace_P) / (det_P + EPS);
    float cond_boost = fminf(0.05f, 0.02f * logf(1.0f + cond));

    return fminf(0.99f, base + entropy_boost + cond_boost);
}

// === Log-Cholesky Geodesic Projection ===
__device__ void log_cholesky_geodesic_projection(float* Pxx, float* Pxy, float* Pxz,
                                                 float* Pyy, float* Pyz, float* Pzz, float entropy_force) {
    ...
    float detp = det_P(pxx, *Pxy, *Pxz, pyy, *Pyz, pzz);
    float trace_P = pxx + pyy + pzz;

    float strength = compute_geodesic_strength(entropy_force, trace_P, detp);
...
// === Enforcement ===
__device__ void enforce_realizability(float* rho, float* P, float* q, float* r, float entropy_force) {
    *rho = fmaxf(*rho, EPS);

    log_cholesky_geodesic_projection(&P[0], &P[1], &P[2], &P[3], &P[4], &P[5], entropy_force);
...
__global__ void ct_entropy_solver_14momentSOTA(
    ...
    enforce_realizability(&rho[idx], P, q, &r, entropy_force);
