...
Jx = cp.zeros((Ni, Ni, Ni), dtype=cp.float32)
Jy = cp.zeros((Ni, Ni, Ni), dtype=cp.float32)
Jz = cp.zeros((Ni, Ni, Ni), dtype=cp.float32)
...
Bz[NG:NG+N, NG:NG+N, NG:NG+N] = cp.asarray(np.random.randn(N, N, N) * pert * 0.5 + 0.7, dtype=cp.float32)
...
R = cp.sqrt(X**2 + Y**2)
R = cp.maximum(R, 0.15)
v_theta = 0.25 * (1.0 - cp.exp(-R / 0.35))

theta = cp.arctan2(Y, X)
vx_seed = -v_theta * cp.sin(theta)
vy_seed =  v_theta * cp.cos(theta)

mx[NG:NG+N, NG:NG+N, NG:NG+N] = rho[NG:NG+N, NG:NG+N, NG:NG+N] * vx_seed[:, :, None]
my[NG:NG+N, NG:NG+N, NG:NG+N] = rho[NG:NG+N, NG:NG+N, NG:NG+N] * vy_seed[:, :, None]

# ====================== KERNELS ======================
compute_current_kernel = cp.RawKernel(r'''
#define NG 3
extern "C" __global__ void compute_current_kernel(
    const float* Bx, const float* By, const float* Bz,
    float* Jx, float* Jy, float* Jz, int Ni, float dx)
{
    ...
    Jx[idx] = ((By[i*Ni*Ni + (j+1)*Ni + k] - By[i*Ni*Ni + j*Ni + k]) -
               (Bz[i*Ni*Ni + j*Ni + (k+1)] - Bz[i*Ni*Ni + j*Ni + k])) * inv_dx;

    Jy[idx] = ((Bz[i*Ni*Ni + j*Ni + (k+1)] - Bz[i*Ni*Ni + j*Ni + k]) -
               (Bx[(i+1)*Ni*Ni + j*Ni + k] - Bx[i*Ni*Ni + j*Ni + k])) * inv_dx;

    Jz[idx] = ((Bx[(i+1)*Ni*Ni + j*Ni + k] - Bx[i*Ni*Ni + j*Ni + k]) -
               (By[i*Ni*Ni + (j+1)*Ni + k] - By[i*Ni*Ni + j*Ni + k])) * inv_dx;
}
''', 'compute_current_kernel')
    ...
    float* Emfx, float* Emfy, float* Emfz, int Ni, float hall_coeff)
{
    ...
    if (i < Ni-1 && j < Ni-1 && k < Ni) {
        ...
        float Emfz_ideal = -(vx_e*By_e - vy_e*Bx_e);

        float Jx_e = 0.25f*(Jx[idx] + Jx[i*Ni*Ni+(j+1)*Ni+k] + Jx[(i+1)*Ni*Ni+j*Ni+k] + Jx[(i+1)*Ni*Ni+(j+1)*Ni+k]);
        float Jy_e = 0.25f*(Jy[idx] + Jy[i*Ni*Ni+(j+1)*Ni+k] + Jy[(i+1)*Ni*Ni+j*Ni+k] + Jy[(i+1)*Ni*Ni+(j+1)*Ni+k]);
        float rho_e = 0.25f*(rho[idx] + rho[i*Ni*Ni+(j+1)*Ni+k] + rho[(i+1)*Ni*Ni+j*Ni+k] + rho[(i+1)*Ni*Ni+(j+1)*Ni+k]);

        float hall_emfz = (hall / fmaxf(rho_e,1e-8f)) * (Jx_e*Bx_e + Jy_e*By_e);
        Emfz[i*Ni*Ni + j*Ni + k] = Emfz_ideal + hall_emfz;
    }
    ...
    if (i < Ni-1 && k < Ni-1) {
        ...
        float Emfy_ideal = -(vz_e*Bx_e - vx_e*Bz_e);

        float Jx_e = 0.25f*(Jx[idx] + Jx[i*Ni*Ni+j*Ni+(k+1)] + Jx[(i+1)*Ni*Ni+j*Ni+k] + Jx[(i+1)*Ni*Ni+j*Ni+(k+1)]);
        float Jz_e = 0.25f*(Jz[idx] + Jz[i*Ni*Ni+j*Ni+(k+1)] + Jz[(i+1)*Ni*Ni+j*Ni+k] + Jz[(i+1)*Ni*Ni+j*Ni+(k+1)]);
        float rho_e = 0.25f*(rho[idx] + rho[i*Ni*Ni+j*Ni+(k+1)] + rho[(i+1)*Ni*Ni+j*Ni+k] + rho[(i+1)*Ni*Ni+j*Ni+(k+1)]);

        float hall_emfy = (hall / fmaxf(rho_e,1e-8f)) * (Jx_e*Bx_e + Jz_e*Bz_e);
        Emfy[i*Ni*Ni + j*Ni + k] = Emfy_ideal + hall_emfy;
    }
    ...
    if (j < Ni-1 && k < Ni-1) {
        ...
        float Emfx_ideal = -(vy_e*Bz_e - vz_e*By_e);

        float Jy_e = 0.25f*(Jy[idx] + Jy[i*Ni*Ni+(j+1)*Ni+k] + Jy[i*Ni*Ni+j*Ni+(k+1)] + Jy[i*Ni*Ni+(j+1)*Ni+(k+1)]);
        float Jz_e = 0.25f*(Jz[idx] + Jz[i*Ni*Ni+(j+1)*Ni+k] + Jz[i*Ni*Ni+j*Ni+(k+1)] + Jz[i*Ni*Ni+(j+1)*Ni+(k+1)]);
        float rho_e = 0.25f*(rho[idx] + rho[i*Ni*Ni+(j+1)*Ni+k] + rho[i*Ni*Ni+j*Ni+(k+1)] + rho[i*Ni*Ni+(j+1)*Ni+(k+1)]);

        float hall_emfx = (hall / fmaxf(rho_e,1e-8f)) * (Jy_e*By_e + Jz_e*Bz_e);
        Emfx[i*Ni*Ni + j*Ni + k] = Emfx_ideal + hall_emfx;
    }
}
''', 'ct_emf_kernel')
...
print("✅ Hall-MHD CT-Yee simulation started\n")

steps = 0
while steps < max_steps:
    update_ghosts()
    compute_current_kernel(grid, block, (Bx, By, Bz, Jx, Jy, Jz, Ni, dx))
    ...
    max_v = float(cp.max(cp.sqrt(v2)))
    max_B = float(cp.sqrt(cp.max(B2)))
    max_J = float(cp.sqrt(cp.max(J2)))
    kin_E = float(0.5 * cp.sum(rho_safe[NG:NG+N,NG:NG+N,NG:NG+N] * v2[NG:NG+N,NG:NG+N,NG:NG+N]))
    mag_E = float(0.5 * cp.sum(B2[NG:NG+N,NG:NG+N,NG:NG+N]))
    ...
    cs2 = gamma * p / rho_c
    cf2 = B2[NG:NG+N,NG:NG+N,NG:NG+N] / rho_c
    cmax = cp.sqrt(cp.max(v2[NG:NG+N, NG:NG+N, NG:NG+N] + cs2 + cf2)).item()
    dt = min(cfl * dx / (cmax + 1e-12), dt_max)
    dt_over_dx = dt / dx

    # STAGE 1
    ct_emf_kernel(grid, block, (rho, mx, my, mz, Bx, By, Bz, Jx, Jy, Jz, Emfx, Emfy, Emfz, Ni, hall_coeff), shared_mem=3*(8+2)*(8+2)*(2+2)*4)
    ...
    # STAGE 2
    ct_emf_kernel(grid, block, (rho1, mx1, my1, mz1, Bx1, By1, Bz1, Jx, Jy, Jz, Emfx, Emfy, Emfz, Ni, hall_coeff), shared_mem=3*(8+2)*(8+2)*(2+2)*4)
    ...
    if steps % print_interval == 0:
        ...
        print(f"Step {steps:4d} | dt={dt:.2e} | Max|v|={max_v:.4f} | Max|B|={max_B:.4f} | Max|J|={max_J:.4f}")
        print(f"   Kin_E={kin_E:.4f} | Mag_E={mag_E:.4f} | mean|divB|={mean_divB:.2e}")
