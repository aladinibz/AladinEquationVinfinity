...
print("🌌 Plasma Cosmology v8.2 — 128³ A100 GPU + Non-linear Alpha Quenching")
...
B_eq = cp.float32(5e-6)   # equipartition ~5 μG
...
def run_simulation(use_dm, use_cr=True):
    ...
    for step in range(steps):
        ...
        # CT + dynamo with **non-linear quenching**
        ...
        alpha = alpha / (1 + B2 / B_eq**2)   # <--- saturation quenching
        Ez[1:,1:,:] += alpha * Bz_c
        ...
        Fx = JxB_x + g_x + tension_r + cent_x - nu_visc * cp.gradient(cp.gradient(vx, dx, axis=0), dx, axis=0)
        Fy = JxB_y + g_y - nu_visc * cp.gradient(cp.gradient(vy, dx, axis=1), dx, axis=1)
        Fz = JxB_z + g_z - nu_visc * cp.gradient(cp.gradient(vz, dx, axis=2), dx, axis=2)
    ...
    a_JxB_avg = a_JxB_avg / (counts + 1e-8)
    
    print("\nJ × B radial acceleration (m/s²):")
    print("Inner (0-5 kpc) avg =", np.mean(np.abs(a_JxB_avg[:10])), "m/s²")
    print("Mid (5-15 kpc) avg =", np.mean(np.abs(a_JxB_avg[10:30])), "m/s²")
    print("Outer (15-30 kpc) avg =", np.mean(np.abs(a_JxB_avg[30:])), "m/s²")
    
    return {'r': r_centers, 'v_phi': v_rot, 'a_JxB': a_JxB_avg}
...
plt.subplot(1, 2, 2)
plt.plot(plasma_data['r'], np.abs(plasma_data['a_JxB']), 'purple', lw=2.5, label='Pure Plasma J×B')
plt.plot(dm_data['r'], np.abs(dm_data['a_JxB']), 'red', lw=2.5, label='DM mode J×B')
plt.xlabel('Radius (kpc)')
plt.ylabel('|J × B radial accel| (m/s²)')
plt.title('J × B Radial Force Profile')
plt.legend()
plt.grid(True)

plt.tight_layout()
plt.show()
plt.savefig('JxB_profile_v8.2.png')

print("✅ v8.2 complete! Share the new J×B radial averages and rotation curve summary.")
