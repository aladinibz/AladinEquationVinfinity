...
# Start close to n=0.25
res = minimize(chi2_both, [75, 0.25], method='L-BFGS-B', bounds=[(70,80), (0.2,0.3)])
...
print(f"n = {n_fit:.3f}, H0 = {H0_fit:.1f}, χ² = {chi2_min:.2f}, p = {p_value:.2e}")

plt.figure(figsize=(9,6))
plt.errorbar(z,H,e,fmt='ko',capsize=5,label='Data')
plt.plot(z,H_fit,'gold',lw=3,label=f'Aladin (n={n_fit:.3f})')
plt.xlabel('z'); plt.ylabel('H(z)')
plt.title('Hubble Tension — n=0.25 Fixed')
plt.legend()
plt.grid(alpha=0.3)
plt.tight_layout()
