"""
ALADIN v0.7 — FULL 3D FIXED (BROADCAST ERROR SOLVED + 9 PLOTS)
Broadcast shape issue fixed with roll-based gradients
...
n_steps = 2000   # quick test — increase later
save_every = 2000
...
flux_r = np.zeros_like(R)
flux_theta = np.zeros_like(R)
flux_z = np.zeros_like(R)

for step in tqdm(range(n_steps), desc="v0.7 Fixed 3D Run", unit="step"):
    try:
        ...
        # Fixed shape-safe current density
        grad_Bz_dtheta = (np.roll(B_z, -1, axis=1) - np.roll(B_z, 1, axis=1)) / (2 * dtheta)
        grad_Btheta_dz = (np.roll(B_theta, -1, axis=2) - np.roll(B_theta, 1, axis=2)) / (2 * dz)
        J_r = (1 / mu0) * (inv_R * grad_Bz_dtheta - grad_Btheta_dz)

        grad_Br_dz = (np.roll(B_r, -1, axis=2) - np.roll(B_r, 1, axis=2)) / (2 * dz)
        grad_Bz_dr = (np.roll(B_z, -1, axis=0) - np.roll(B_z, 1, axis=0)) / (2 * dr)
        J_theta = (1 / mu0) * (grad_Br_dz - grad_Bz_dr)

        grad_RBtheta_dr = (np.roll(R * B_theta, -1, axis=0) - np.roll(R * B_theta, 1, axis=0)) / (2 * dr)
        grad_Br_dtheta = (np.roll(B_r, -1, axis=1) - np.roll(B_r, 1, axis=1)) / (2 * dtheta)
        J_z = (1 / mu0) * (inv_R * grad_RBtheta_dr - inv_R * grad_Br_dtheta)
...
print("v0.7 FULL 3D run complete — 9 publish plots generated!")
