...
# Load subject_23 .edf (update path if needed)
edf_path = 'data/raw_edf/subject_23_dmt_35mg.edf'
...
print(f"Higuchi FD on subject_23: {hfd:.4f}")

# Plot + save
plt.figure(figsize=(12,6))
plt.plot(times, data[0], 'gold', lw=2)
plt.title(f"Subject 23 (DMT) — Full EEG\nHiguchi FD = {hfd:.3f}")
plt.xlabel("Time [s]"); plt.ylabel("Amplitude [µV]")
plt.grid(alpha=0.3); plt.tight_layout()
plt.savefig("subject_23_higuchi_fd.png", dpi=400)
plt.show()
