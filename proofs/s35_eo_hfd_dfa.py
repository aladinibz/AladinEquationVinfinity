...
raw = mne.io.read_raw_bdf('S35-EO.bdf', preload=True)
...
hfd_eo = higuchi_fd(data[0])
dfa_eo = dfa(data[0])
print(f"S35-EO: HFD = {hfd_eo:.4f}, DFA α = {dfa_eo:.4f}")

plt.figure(figsize=(12,8),dpi=1200)
plt.bar(['HFD', 'DFA α'], [hfd_eo, dfa_eo], color='green')
plt.title("Subject 35 — Eyes Open Baseline\nHFD + DFA α",fontsize=18)
plt.ylabel("Value")
plt.grid(alpha=0.4); plt.tight_layout()
plt.savefig("plots/s35_eo_hfd_dfa.png",dpi=1200)
print("Saved: plots/s35_eo_hfd_dfa.png")
plt.show()
