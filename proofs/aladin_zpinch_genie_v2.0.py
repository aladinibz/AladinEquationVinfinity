...
from scipy.special import j0  # for Bessel J0

GRID_SIZE = 128
MAX_STEPS = 1000           # short test – quick feedback
dt_max    = 1.2e-4
CFL       = 0.75

print("🚀 ALADIN v1.6 – 1000 steps TEST – BESSEL + m=1&2 KINK")
print("Bessel J₀(kr) + light m=1 & m=2 kink seed – symmetry break test")
print(f"dt_max = {dt_max:.1e}, CFL = {CFL}, expected runtime ~1–2 minutes")
...
# Bessel J0(kr) radial profile – repo eigenmode
r_1d = r.get()
...
# Light kink seed m=1 & m=2 only – symmetry break
rho += 0.05 * rho0 * cp.cos(1 * Phi) * cp.cos(k_sausage * Z)  # m=1
rho += 0.05 * rho0 * cp.cos(2 * Phi) * cp.cos(k_sausage * Z)  # m=2
v_phi += 0.02 * vA * cp.sin(1 * Phi) * cp.sin(k_sausage * Z)  # m=1
v_phi += 0.02 * vA * cp.sin(2 * Phi) * cp.sin(k_sausage * Z)  # m=2
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.6 – 1000 steps TEST – BESSEL + m=1&2 KINK")
...
while t < t_max and step < MAX_STEPS:
    ...
    if step % 500 == 0:  # print every 500 for short test
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.6 – 1000 steps TEST – FINAL ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    plt.title('Clean 43 Hz Bandpass Filtered Signals – 1000 steps TEST')
    plt.grid(alpha=0.3)
    plt.legend()
    plt.savefig('plots/plot35_filtered_43hz_signals_1000test.png', dpi=200)
    plt.close()
    print("Saved: plots/plot35_filtered_43hz_signals_1000test.png")
else:
    print("History too short for bandpass – skipping 43 Hz analysis")
...
plt.ylabel('Power')
plt.title('Fermion Mass z-Spatial Power Spectrum – 1000 steps TEST')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_1000test.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_1000test.png")
...
plt.ylabel('Power')
plt.title('Azimuthal Power Spectrum – Kink Detection (1000 steps TEST)')
plt.axvline(1, color='red', ls='--', label='m=1 kink')
plt.legend()
plt.grid(alpha=0.3)
plt.savefig('plots/plot45_azimuthal_power_kink_1000test.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot45_azimuthal_power_kink_1000test.png")

print("\nALADIN v1.6 – 1000 steps TEST finished!")
print("Bessel seed + m=1,2 kink seed active – check sausage band and m=1 power!")
