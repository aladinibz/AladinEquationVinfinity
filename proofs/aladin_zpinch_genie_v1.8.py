...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.8 – 50,000 steps – 256³ HIGH-RES PROOF         ║
# ║  (256³ grid, Bessel J₀(kr) seed, zero diffusion, max kink/sausage)         ║
# ╚════════════════════════════════════════════════════════════════════════════╝

GRID_SIZE = 256
MAX_STEPS = 50000          # short high-res proof run
dt_max    = 1.2e-4
CFL       = 0.75

print("🚀 ALADIN v1.8 – 50k steps – 256³ HIGH-RES PROOF TEST")
print("High-res grid + Bessel J₀(kr) + zero diffusion – spatial modes should finally show!")
...
k_sausage = cp.float64(0.7)  # repo max growth
...
# Initial conditions – BESSEL J₀(kr) + MAX KINK SEED
...
# Max kink seed
v_phi += 0.8 * vA * cp.sin(Phi) * cp.cos(k_sausage * Z)
v_r += 0.4 * vA * cp.cos(Phi) * cp.sin(k_sausage * Z)
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.8 – 50k on 256³ – BESSEL & KINK PROOF")
...
while t < t_max and step < MAX_STEPS:
    ...
    majorana_backreaction = 5e-4 * ferm_total_mass  # strong push
    ...
    genie_phi_new -= 0.0 * cyl_laplacian(genie_phi_new) * dt  # ZERO diffusion
    ...
    if step % 5 == 0:
        ...
        low = cp.percentile(fields, 0.05)   # tighter for memory
        high = cp.percentile(fields, 99.95)
    ...
    if step % 10000 == 0 and step > 0:  # less frequent on high-res
...
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.8 – 50,000 steps on 256³ – FINAL ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    plt.title('Clean 43 Hz Bandpass Filtered Signals – 256³ Proof')
    plt.grid(alpha=0.3)
    plt.legend()
    plt.savefig('plots/plot35_filtered_43hz_signals_256_50k.png', dpi=200)
    plt.close()
    print("Saved: plots/plot35_filtered_43hz_signals_256_50k.png")
...
plt.ylabel('Power')
plt.title('Fermion Mass z-Spatial Power Spectrum – 256³ 50k')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_256_50k.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_256_50k.png")
...
plt.ylabel('Power')
plt.title('Azimuthal Power Spectrum – Kink Detection (256³ 50k)')
plt.axvline(1, color='red', ls='--', label='m=1 kink')
plt.legend()
plt.grid(alpha=0.3)
plt.savefig('plots/plot45_azimuthal_power_kink_256_50k.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot45_azimuthal_power_kink_256_50k.png")

print("\nALADIN v1.8 – 50,000 steps on 256³ finished!")
print("High-res + Bessel seed + zero diffusion active – check sausage & m=1 power!")
