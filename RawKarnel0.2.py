...
print("🌌 ALADIN Plasma Cosmology v38.8 — STABLE FUSED RAWKERNEL TRUE CT YEE")
                         ...
                         float dt, float dx, int N, float c_h, float kappa)
{
    ...
    if (i < N && j < N && k < N) {
        ...
        s_vy[s] = vy[g];
        s_vz[s] = vz[g];
        s_Bx[s] = Bx[g];      // note: Bx is staggered but we load nearest
        s_By[s] = By[g];
        s_Bz[s] = Bz[g];
    }
    __syncthreads();

    // ====================== TRUE CT STAGGERED CURL ======================
    if (i < N-1 && j < N-1 && k < N-1) {
        ...
        float Ez_val = -(vx_avg * By_avg - vy_avg * Bx_avg);
...
print("Starting v38.8 with FULL FUSED RAWKERNEL CT YEE...")
...
print("\n✅ v38.8 Finished! Fused RawKernel with staggered CT structure.")
