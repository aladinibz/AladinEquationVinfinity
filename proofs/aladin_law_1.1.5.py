"""
ALADIN v1.1.5 — Relativistic Jet Spine + Radial Modal Profiles
128³ Locked
"""
...
print("🚀 ALADIN v1.1.5 — Relativistic Jet Spine + Radial Profiles\n")
...
# ========================= CONSTANTS + RELATIVISTIC JET =========================
mu0 = 4 * np.pi * 1e-7
c = 3.0e8
m_e = 9.109e-31
gamma_ad = 5.0/3

GAMMA_BULK = 5.0          # ← Change this for different jet speeds (1.0 = non-rel)
...
for J0 in J0_list:
    for rho0 in rho0_list:
        ...
        # Jet-like axial spine
        v_z += 0.8 * c * np.exp(-(R / (0.4 * r_max))**2)   # strong central jet

        v_r += 0.003 * np.sin(Theta) * np.exp(-((R-0.8)**2)/0.25)
        ...
        for step in tqdm(range(n_steps), leave=False, desc=f"J0={J0:.1e} γ={GAMMA_BULK}"):
            ...
            if step % save_every == 0:
                ...
                if step % (save_every * 5) == 0:
                    vr_fft = np.fft.fft(v_r, axis=1)
                    power_m_r = np.sum(np.abs(vr_fft)**2, axis=2)   # (nr, ntheta)

                    radial_m0 = np.sum(power_m_r[:, 0:1] * R[:,0,np.newaxis] * dr, axis=1)
                    radial_m1 = np.sum(power_m_r[:, 1:2] * R[:,0,np.newaxis] * dr, axis=1)

                    fig, axs = plt.subplots(1, 2, figsize=(14, 5))
                    axs[0].plot(r, radial_m0, 'b-', lw=2, label='m=0 Sausage')
                    axs[0].set_title('Radial Modal Power - m=0')
                    axs[0].set_xlabel('r')
                    axs[0].legend()

                    axs[1].plot(r, radial_m1, 'r-', lw=2, label='m=1 Kink')
                    axs[1].set_title('Radial Modal Power - m=1')
                    axs[1].set_xlabel('r')
                    axs[1].legend()

                    plt.suptitle(f'J0={J0:.1e} | γ={GAMMA_BULK} | t={t:.2e} | Π={Pi:.2f}')
                    plt.tight_layout()
                    plt.savefig(f'jet_plots/radial_modes_J0{int(J0/1e5)}e5_step{step:04d}.png', dpi=180)
                    plt.close()
...
plt.xlabel('Π', fontsize=14)
plt.ylabel('Growth Rate γ (s⁻¹)', fontsize=14)
plt.title(f'Relativistic Jet (γ = {GAMMA_BULK})')
plt.legend()
plt.grid(alpha=0.3)
plt.show()

print("\nRadial eigenmode profiles saved in 'jet_plots/' folder")
