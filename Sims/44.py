...
# ====================== PARAMETERS ======================
N = 128   # smaller for stability, increase later if it runs
L = 60.0
dx = L / N
...
M_dm = nfw_enclosed_mass(r3d)   # define function if needed
g_r = -G * M_dm / r3d**2
...
for step in range(steps):
    ...
    dt = 0.3 * dx / 200.0   # safe dt for now
    ...
    if step % 50 == 0:
        divB_max = float(compute_divB())   # define this function
        vmax = float(cp.max(cp.sqrt(vx**2 + vy**2 + vz**2)))
        Bmax = float(cp.max(cp.sqrt(Bx**2 + By**2 + Bz**2)))
        print(f"Step {step:4d} | Bmax = {Bmax:.2f} | vmax = {vmax:.1f} | divB = {divB_max:.2e}")
