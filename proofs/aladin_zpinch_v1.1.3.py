# mega ultra maniac v1.1.3 – COLAB-READY FINAL FROZEN PUBLISH VERSION
...
!pip install --upgrade "jax[cuda12_pip]" matplotlib pyvista scipy tqdm -q 2>/dev/null
...
# ─── MAIN PLOTS (13 + energy breakdown) ─────────────────────────────────────
...
# Extra: Energy breakdown
fig = plt.figure(figsize=(12, 6), facecolor='black')
ax = fig.gca()
ax.set_facecolor('black')
ax.stackplot(real_time[:-1], 
             [np.array(diagnostics['total_energy']) - diagnostics['E_mag'] - diagnostics['E_int'],
              diagnostics['E_mag'],
              diagnostics['E_int']],
...
# ... (add remaining 10 plots with units as before - omitted for brevity)
...
fig.savefig('plots/brio_wu_validation.png', dpi=CONFIG['dpi'], facecolor='black')
plt.close(fig)
...
plt.tight_layout()
fig.savefig('plots/orszag_tang_density.png', dpi=CONFIG['dpi'], facecolor='black')
plt.close(fig)
...
ax.grid(True)
ax.legend()
fig.savefig('plots/hydro_blast_wave.png', dpi=CONFIG['dpi'])
plt.close(fig)
...
ax.grid(True)
ax.legend()
fig.savefig('plots/mhd_blast_wave_validation.png', dpi=CONFIG['dpi'])
plt.close(fig)
...
ax.set_xlabel('x')
ax.set_ylabel('y')
fig.savefig('plots/gem_reconnection_density.png', dpi=CONFIG['dpi'])
plt.close(fig)
...
ax.set_xlabel('x')
ax.set_ylabel('y')
fig.savefig('plots/gem_reconnection_current.png', dpi=CONFIG['dpi'])
plt.close(fig)
...
fig_sp.savefig('plots/sweet_parker_reconnection_full.png', dpi=CONFIG['dpi'], facecolor='black')
plt.close(fig_sp)
...
def run_integration_tests():
    ...
    # 2. Orszag-Tang integration test
    ...
    # 3. Hydro Blast Wave integration test
    ...
    # 4. MHD Blast Wave integration test
    ...
    J_final = (grad_y(state_gem['B'][1]) - grad_x(state_gem['B'][0])) / CONFIG['mu0']
    ...
    # 6. Sweet-Parker integration test
