# mega ultra maniac v1.0.6 – FROZEN FINAL PUBLISH VERSION
# 3D Resistive MHD Z-pinch Kink POC (128³) + 4 validations + resolution & growth-rate convergence
# Finite-difference scheme with hyperbolic divergence cleaning + constant resistivity
# Line-tied BCs, 13 plots + convergence plots + analytic m=3 growth overlay – Zenodo ready
...
# Analytic m=3 kink growth rate (simplified low-β Z-pinch approximation)
k_z = CONFIG['k_pert'] / 0.1
v_A_approx = CONFIG['v_max'] * 0.5
gamma_analytic = k_z * v_A_approx * np.sqrt(3 * 2 / 4)  # rough m(m-1)/(m+1) for m=3
print(f"Analytic m=3 growth rate (approx): {gamma_analytic:.2e} /s")
...
# ────────────────────────────────────────────────
# Run main Z-pinch simulation (128³)
# ────────────────────────────────────────────────
states = [state]
...
print("Running main Z-pinch simulation (128³)...")
...
print("\nAll done – v1.0.6 Final is frozen.")
print("Check 'plots/' folder for all outputs.")
print("Ready for GitHub + Zenodo – legend status achieved!")
