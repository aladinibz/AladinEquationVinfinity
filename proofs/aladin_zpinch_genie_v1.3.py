...
# ╔════════════════════════════════════════════════════════════════════════════╗
# ║                  ALADIN v1.3 – 25,000 steps – ALL IN                        ║
# ║  (128³ grid, max dt ramp, Majorana domination, 43 Hz + spectra)            ║
# ╚════════════════════════════════════════════════════════════════════════════╝

TEST_MODE = True

GRID_SIZE = 128
MAX_STEPS = 25000          # ALL IN – 25k steps
dt_max    = 2e-5           # 20 µs ceiling
CFL       = 0.45           # aggressive but monitored

print("🚀 ALADIN v1.3 – 25,000 steps – ALL IN MODE")
print(f"dt_max = {dt_max:.1e}, CFL = {CFL}, expected runtime ~15–30 min on A100")
...
progress_bar = tqdm(total=MAX_STEPS, desc="ALADIN v1.3 – 25k steps – all in")
...
while t < t_max and step < MAX_STEPS:
    ...
    # SUPER aggressive ramp-up: first 10k steps push dt HARD
    if step < 10000 and t < 0.05:
        dt_new = cp.maximum(dt_new, 2e-4)          # min 200 µs early
        if t < 0.01:
            dt_new = cp.minimum(dt_new * 1.8, 4e-4)  # very strong exponential

    dt = 0.7 * dt_prev + 0.3 * dt_new
    dt = cp.minimum(dt, dt_max)
    dt = cp.maximum(dt, 1e-7)  # solid floor
    ...
    if current_amp > 0.8:
        local_damping *= 2.5  # strong emergency damping

    genie_phi_new *= cp.exp(-local_damping * dt)
    genie_phi_new -= 0.01 * cyl_laplacian(genie_phi_new) * dt  # slightly stronger diffusion
    genie_phi_new = cp.clip(genie_phi_new, -1.2, 1.2)
    ...
    C_field = cp.clip(C_field, 0.0, 12.0)
...
# =============================================================================
# POST-PROCESSING – fixed slice error + safety
# =============================================================================
print("\n" + "="*80)
print("ALADIN v1.3 – 20,000 steps max – FINAL ANALYSIS")
print("="*80)
...
if len(genie_center_history) > 200:
    ...
    plt.title(f'43 Hz Resonance Detection – 20000 steps max – {verdict}', color='white')
    plt.legend()
    plt.grid(alpha=0.25, which='both', color='gray')
    plt.savefig('plots/plot43hz_detection_v13_20000steps.png', dpi=220, facecolor='black')
    plt.close()
    print("Saved: plots/plot43hz_detection_v13_20000steps.png")
else:
...
plt.ylabel('Power')
plt.title('Fermion Mass Spatial Power Spectrum – 20000 steps max')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot43_fermion_mass_spatial_spectrum_20000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot43_fermion_mass_spatial_spectrum_20000.png")
...
plt.xlabel('Wavenumber k (1/a)')
plt.ylabel('Power')
plt.title('Fermion Density Spatial Power Spectrum – 20000 steps max')
plt.legend()
plt.grid(alpha=0.3, which='both')
plt.savefig('plots/plot44_fermion_density_spatial_spectrum_20000.png', dpi=200, facecolor='black')
plt.close()
print("Saved: plots/plot44_fermion_density_spatial_spectrum_20000.png")
...
print("\nALADIN v1.3 – 20,000 steps max finished!")
print("Drop the final time span, 43 Hz verdict, dominant k, Majorana %, genie amp, and any plot vibes.")
